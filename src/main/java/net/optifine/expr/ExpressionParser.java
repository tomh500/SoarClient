...
public class ExpressionParser {
	...
	private void mergeOperators(List<IExpression> listExpr, List<FunctionType> listFuncs, int precedence)
	...
	private IExpression[] parseExpressions(Deque<Token> deque) throws ParseException {
		...
		while (true) {
			...
			if (iexpression == null) {
				IExpression[] aiexpression = (IExpression[]) ((IExpression[]) list
						.toArray(new IExpression[list.size()]));
				return aiexpression;
			}
		...
		if (args.length != aexpressiontype.length) {
			...
			for (int i = 0; i < args.length; ++i) {
				...
				if (expressiontype != expressiontype1) {
					throw new ParseException("Invalid argument type, function: \"" + type.getName() + "\", index: " + i
							+ ", type: " + expressiontype + ", should be: " + expressiontype1);
				}
			}
			...
			if (type.getExpressionType() == ExpressionType.FLOAT) {
				...
				throw new ParseException(
						"Unknown function type: " + type.getExpressionType() + ", function: " + type.getName());
			}
		}
	}
	...
	private static Deque<Token> getGroup(Deque<Token> deque, TokenType tokenTypeEnd, boolean tokenEndRequired)
			throws ParseException {
