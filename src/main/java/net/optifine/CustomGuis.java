...
public class CustomGuis {
		...
		if (guiProperties == null) {
			...
			if (!(guiscreen instanceof GuiContainer)) {
				return loc;
					...
					&& loc.getResourcePath().startsWith("textures/gui/")) {
				if (playerControllerOF == null) {
					...
					if (iblockaccess == null) {
						...
						return getTexturePos(CustomGuiProperties.EnumContainer.CREATIVE, mc.thePlayer.getPosition(),
								iblockaccess, loc, guiscreen);
					} else if (guiscreen instanceof GuiInventory) {
						return getTexturePos(CustomGuiProperties.EnumContainer.INVENTORY, mc.thePlayer.getPosition(),
								iblockaccess, loc, guiscreen);
					} else {
	...
	private static ResourceLocation getTexturePos(CustomGuiProperties.EnumContainer container, BlockPos pos,
			IBlockAccess blockAccess, ResourceLocation loc, GuiScreen screen) {
	...
	private static ResourceLocation getTextureEntity(CustomGuiProperties.EnumContainer container, Entity entity,
			IBlockAccess blockAccess, ResourceLocation loc) {
		CustomGuiProperties[] acustomguiproperties = guiProperties[container.ordinal()];
		...
		if (listProps.isEmpty()) {
			...
			for (int i = 0; i < acustomguiproperties.length; ++i) {
				if (listProps.size() > i) {
					...
					if (list != null) {
						CustomGuiProperties[] acustomguiproperties1 = (CustomGuiProperties[]) ((CustomGuiProperties[]) list
	...
	private static void update(IResourcePack rp, List<List<CustomGuiProperties>> listProps) {
		String[] astring = ResUtils.collectFiles(rp, (String) "optifine/gui/container/", (String) ".properties",
				(String[]) null);
