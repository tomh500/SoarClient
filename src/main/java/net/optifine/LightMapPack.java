		...
		if (this.lightMapRain == null && this.lightMapThunder == null) {
			...
			if (i != 1 && i != -1) {
				...
				if (!flag && !flag1) {
					...
					if (f4 > f2 && this.lightMapRain != null
							&& this.lightMapRain.updateLightmap(world, torchFlickerX, aint[j], nightvision)) {
						afloat[j] = f4;
						++j;
					}

					if (f1 > f2 && this.lightMapThunder != null
							&& this.lightMapThunder.updateLightmap(world, torchFlickerX, aint[j], nightvision)) {
						afloat[j] = f1;
						++j;
					}

					return j == 2 ? this.blend(aint[0], afloat[0], aint[1], afloat[1])
