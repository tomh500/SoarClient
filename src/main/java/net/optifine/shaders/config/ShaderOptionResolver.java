...
public class ShaderOptionResolver implements IExpressionResolver {
	...
	public ShaderOptionResolver(ShaderOption[] options) {
		for (int i = 0; i < options.length; ++i) {
			...
			if (shaderoption instanceof ShaderOptionSwitch) {
				...
				ExpressionShaderOptionSwitch expressionshaderoptionswitch = new ExpressionShaderOptionSwitch(
						shaderoptionswitch);
	...
	public IExpression getExpression(String name) {
		ExpressionShaderOptionSwitch expressionshaderoptionswitch = (ExpressionShaderOptionSwitch) this.mapOptions
				.get(name);
