...
public class ShadowUtils {
	public static Iterator<RenderChunk> makeShadowChunkIterator(WorldClient world, double partialTicks,
		...
		if (f > 0.0F && f < (float) ((renderDistanceChunks - 1) * 16)) {
			...
			BlockPos blockpos1 = blockpos.add(-f3 * (float) i, -f4 * (float) i,
                    -f5 * (float) i);
			BlockPos blockpos2 = blockpos.add(f3 * (float) renderDistanceChunks,
                    f4 * (float) renderDistanceChunks, f5 * (float) renderDistanceChunks);
