...
public class CustomBlockLayers {
	...
	private static void readLayer(String name, EnumWorldBlockLayer layer, Properties props,
			List<EnumWorldBlockLayer> listLayers) {
		String s = "layer." + name;
				...
				for (int i = 0; i < amatchblock.length; ++i) {
					...
					if (j > 0) {
						...
						if (listLayers.get(j) != null) {
							Config.warn(
									"CustomBlockLayers: Block layer is already set, block: " + j + ", layer: " + name);
						}
