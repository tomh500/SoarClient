...
public class CommandScoreboard extends CommandBase {
	...
	protected void addObjective(ICommandSender sender, String[] args, int index) throws CommandException {
		...
		if (iscoreobjectivecriteria == null) {
			...
			if (args.length > index) {
				...
				if (s2.length() > 32) {
					throw new SyntaxErrorException("commands.scoreboard.objectives.add.displayTooLong", s2,
							Integer.valueOf(32));
				}
	...
	protected void addTeam(ICommandSender sender, String[] args, int index) throws CommandException {
		...
		if (scoreboard.getTeam(s) != null) {
			...
			if (args.length > index) {
				...
				if (s1.length() > 32) {
					throw new SyntaxErrorException("commands.scoreboard.teams.add.displayTooLong", s1,
							Integer.valueOf(32));
				}
			...
			if (!s.equalsIgnoreCase("color") && !s.equalsIgnoreCase("friendlyfire")
				...
				if (s.equalsIgnoreCase("color")) {
					...
					if (!s.equalsIgnoreCase("nametagVisibility") && !s.equalsIgnoreCase("deathMessageVisibility")) {
						...
						throw new WrongUsageException("commands.scoreboard.teams.option.noValue", s,
								joinNiceString(Team.EnumVisible.func_178825_a()));
					}
				} else {
					throw new WrongUsageException("commands.scoreboard.teams.option.noValue", s,
				...
				notifyOperators(sender, this, "commands.scoreboard.teams.option.success", s,
	...
	protected void listTeams(ICommandSender p_147186_1_, String[] p_147186_2_, int p_147186_3_)
		...
		if (p_147186_2_.length > p_147186_3_) {
			...
			ChatComponentTranslation chatcomponenttranslation = new ChatComponentTranslation(
					"commands.scoreboard.teams.list.player.count", Integer.valueOf(collection.size()),
					scoreplayerteam.getRegisteredName());
		...
		if (i < 0) {
			...
			if (scoreobjective != null) {
				notifyOperators(p_147198_1_, this, "commands.scoreboard.objectives.setdisplay.successSet",
						Scoreboard.getObjectiveDisplaySlot(i), scoreobjective.getName());
			} else {
				notifyOperators(p_147198_1_, this, "commands.scoreboard.objectives.setdisplay.successCleared",
						Scoreboard.getObjectiveDisplaySlot(i));
			}
		}
	}
		...
		if (p_147195_2_.length > p_147195_3_) {
			...
			for (Score score : map.values()) {
				p_147195_1_.addChatMessage(new ChatComponentTranslation("commands.scoreboard.players.list.player.entry",
						Integer.valueOf(score.getScorePoints()), score.getObjective().getDisplayName(),
	...
	protected void setPlayer(ICommandSender p_147197_1_, String[] p_147197_2_, int p_147197_3_)
		...
		if (s1.length() > 40) {
			...
			notifyOperators(p_147197_1_, this, "commands.scoreboard.players.set.success", scoreobjective.getName(), s1,
	...
	protected void func_175778_p(ICommandSender p_175778_1_, String[] p_175778_2_, int p_175778_3_)
		...
		if (s.length() > 40) {
			...
			if (!scoreboard.entityHasObjective(s2, scoreobjective1)) {
				throw new CommandException("commands.scoreboard.players.operation.notFound", scoreobjective1.getName(),
						s2);
			} else {
	...
	public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
		if (args.length == 1) {
			...
			if (args[0].equalsIgnoreCase("objectives")) {
				...
				if (args.length == 2) {
					return getListOfStringsMatchingLastWord(args, "set", "add", "remove", "reset", "list", "enable",
							"test", "operation");
				}
				...
				if (args.length == 2) {
					return getListOfStringsMatchingLastWord(args, "add", "remove", "join", "leave", "empty", "list",
							"option");
				}
				...
				if (args[1].equalsIgnoreCase("join")) {
					...
					if (!args[1].equalsIgnoreCase("empty") && !args[1].equalsIgnoreCase("list")
						...
						if (args[1].equalsIgnoreCase("option")) {
							...
							if (args.length == 4) {
								return getListOfStringsMatchingLastWord(args, "color", "friendlyfire",
