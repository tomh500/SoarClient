		...
		if (!this.mc.running) {
			...
			if (i - this.systemTime >= 100L) {
				...
				if (Reflector.FMLClientHandler_handleLoadingScreen.exists()) {
					...
					if (object != null) {
						flag = !Reflector.callBoolean(object, Reflector.FMLClientHandler_handleLoadingScreen,
								new Object[] { scaledresolution });
					}
				}
				...
				if (flag) {
					...
					if (progress >= 0) {
						...
						worldrenderer.pos((double) (j1 + l1), (double) (k1 + i1), 0.0D).color(128, 128, 128, 255)
								.endVertex();
						...
						worldrenderer.pos((double) (j1 + progress), (double) (k1 + i1), 0.0D).color(128, 255, 128, 255)
								.endVertex();
						worldrenderer.pos((double) (j1 + progress), (double) k1, 0.0D).color(128, 255, 128, 255)
								.endVertex();
						tessellator.draw();
