...
public class GameSettings {
	...
	public static String getKeyDisplayString(int key) {
		return key < 0 ? I18n.format("key.mouseButton", Integer.valueOf(key + 101))
				: (key < 256 ? Keyboard.getKeyName(key)
						: String.format("%c", Character.valueOf((char) (key - 256))).toUpperCase());
	}
			...
			if (settingOption.getEnumFloat()) {
				...
				return settingOption == GameSettings.Options.MIPMAP_LEVELS && (double) f1 >= 4.0D
						...
						: (settingOption == GameSettings.Options.SENSITIVITY
								? (f == 0.0F ? s1 + I18n.format("options.sensitivity.min")
										: (f == 1.0F ? s1 + I18n.format("options.sensitivity.max")
												: s1 + (int) (f * 200.0F) + "%"))
	...
	private String getKeyBindingOF(GameSettings.Options p_getKeyBindingOF_1_) {
		...
		if (p_getKeyBindingOF_1_ == GameSettings.Options.RENDER_DISTANCE) {
			...
			return f == 0.0F ? s + Lang.get("of.options.framerateLimit.vsync")
					: (f == p_getKeyBindingOF_1_.valueMax ? s + I18n.format("options.framerateLimit.max")
							: s + (int) f + " fps");
		} else {
			return null;
		}
