...
public class TileEntityPistonRenderer extends TileEntitySpecialRenderer<TileEntityPiston> {
	...
	public void renderTileEntityAt(TileEntityPiston te, double x, double y, double z, float partialTicks,
		...
		if (!Soarium.getConfig().detailSettings.piston) {
			return;
		}
		...
		if (block.getMaterial() != Material.air && te.getProgress(partialTicks) < 1.0F) {
			...
			worldrenderer.setTranslation((float) x - (float) blockpos.getX() + te.getOffsetX(partialTicks),
					(float) y - (float) blockpos.getY() + te.getOffsetY(partialTicks),
					(float) z - (float) blockpos.getZ() + te.getOffsetZ(partialTicks));
			...
			if (block == Blocks.piston_head && te.getProgress(partialTicks) < 0.5F) {
				...
				worldrenderer.setTranslation((float) x - (float) blockpos.getX(), (float) y - (float) blockpos.getY(),
