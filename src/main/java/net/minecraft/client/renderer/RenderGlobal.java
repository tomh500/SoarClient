...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener {
	...
	public void setWorldAndLoadRenderers(WorldClient worldClientIn) {
		...
		try {
			this.renderer.setWorld(worldClientIn);
		} finally {
	...
	public void renderEntities(Entity renderViewEntity, ICamera camera, float partialTicks) {
		...
		TileEntityRendererDispatcher.instance.cacheActiveRenderInfo(this.theWorld, this.mc.getTextureManager(),
				this.mc.fontRendererObj, renderView, partialTicks);
		this.renderManager.cacheActiveRenderInfo(this.theWorld, this.mc.fontRendererObj, renderView,
				this.mc.pointedEntity, this.mc.gameSettings, partialTicks);
		double renderX = renderView.lastTickPosX + (renderView.posX - renderView.lastTickPosX) * (double) partialTicks;
		double renderY = renderView.lastTickPosY + (renderView.posY - renderView.lastTickPosY) * (double) partialTicks;
		double renderZ = renderView.lastTickPosZ + (renderView.posZ - renderView.lastTickPosZ) * (double) partialTicks;
		...
		this.renderer.renderTileEntities(partialTicks, this.damagedBlocks);
	...
	public void setupTerrain(Entity viewEntity, double partialTicks, ICamera camera, int frameCount,
		...
		try {
			this.renderer.updateChunks((Frustum) camera, (float) partialTicks, hasForcedFrustum, frameCount,
					playerSpectator);
		} finally {
			RenderDevice.exitManagedCode();
		}
	...
	public void renderSky(float partialTicks, int pass) {
		...
		Vec3 cameraPosition = CameraUtil.getCameraPosition(partialTicks);
		Entity cameraEntity = this.mc.getRenderViewEntity();
		boolean hasBlindness = cameraEntity instanceof EntityLivingBase
				&& ((EntityLivingBase) cameraEntity).getActivePotionEffect(Potion.blindness) != null;
		boolean useThickFog = this.mc.theWorld.provider.doesXZShowFog(MathHelper.floor_double(cameraPosition.xCoord),
				MathHelper.floor_double(cameraPosition.yCoord));
