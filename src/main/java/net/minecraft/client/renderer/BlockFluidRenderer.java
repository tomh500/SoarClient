...
public class BlockFluidRenderer {
	...
	public boolean renderFluid(IBlockAccess blockAccess, IBlockState blockStateIn, BlockPos blockPosIn,
		...
		try {
			...
			TextureAtlasSprite[] atextureatlassprite = blockliquid.getMaterial() == Material.lava
			...
			if (flag || flag1 || aboolean[0] || aboolean[1] || aboolean[2] || aboolean[3]) {
				...
				if (flag1) {
					...
					worldRendererIn.pos(d0, d1, d2 + 1.0D).color(f * f41, f1 * f41, f2 * f41, 1.0F)
							.tex((double) f35, (double) f38).lightmap(i2, j2).endVertex();
					worldRendererIn.pos(d0, d1, d2).color(f * f41, f1 * f41, f2 * f41, 1.0F)
							.tex((double) f35, (double) f37).lightmap(i2, j2).endVertex();
					worldRendererIn.pos(d0 + 1.0D, d1, d2).color(f * f41, f1 * f41, f2 * f41, 1.0F)
							.tex((double) f36, (double) f37).lightmap(i2, j2).endVertex();
					worldRendererIn.pos(d0 + 1.0D, d1, d2 + 1.0D).color(f * f41, f1 * f41, f2 * f41, 1.0F)
							.tex((double) f36, (double) f38).lightmap(i2, j2).endVertex();
					flag2 = true;
				}
				...
				for (int i1 = 0; i1 < 4; ++i1) {
					...
					if (aboolean[i1]) {
						...
						int k = j >> 16 & 65535;
						int l = j & 65535;
						float f31 = i1 < 2 ? FaceBakery.getFaceBrightness(EnumFacing.NORTH)
								: FaceBakery.getFaceBrightness(EnumFacing.WEST);
						float f32 = f4 * f31 * f;
						float f33 = f4 * f31 * f1;
						float f34 = f4 * f31 * f2;
						worldRendererIn.pos(d3, d1 + (double) f39, d4).color(f32, f33, f34, 1.0F)
								.tex((double) f42, (double) f28).lightmap(k, l).endVertex();
						...
						worldRendererIn.pos(d3, d1 + 0.0D, d4).color(f32, f33, f34, 1.0F)
								.tex((double) f42, (double) f30).lightmap(k, l).endVertex();
						worldRendererIn.pos(d3, d1 + 0.0D, d4).color(f32, f33, f34, 1.0F)
								.tex((double) f42, (double) f30).lightmap(k, l).endVertex();
						worldRendererIn.pos(d5, d1 + 0.0D, d6).color(f32, f33, f34, 1.0F)
								.tex((double) f27, (double) f30).lightmap(k, l).endVertex();
