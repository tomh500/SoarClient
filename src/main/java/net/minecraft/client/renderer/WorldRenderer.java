...
public class WorldRenderer implements VertexBufferView, VertexDrain
{
	...
	public boolean oldium$ensureBufferCapacity(int bytes) {
		if (this.vertexFormat != null) {
			bytes += this.vertexFormat.getIntegerSize();
		}
	...
	private static void insertionSort(int[] a, int from, int to, float[] dist) {
		...
		while (++i < to) {
			...
			for (int u = a[i - 1]; Floats.compare(dist[u], dist[t]) < 0; u = a[j - 1]) {
				a[j] = u;
				if (from == j - 1) {
					j--;
					break;
	...
	public <T extends VertexSink> T oldium$createSink(VertexType<T> factory) {
		...
		return blittable != null && blittable.getBufferVertexFormat() == this.oldium$getVertexFormat()
			? blittable.createBufferWriter(this, SodiumClientMod.isDirectMemoryAccessEnabled())
			: factory.createFallbackWriter((WorldRenderer)this);
	}
}
