...
public class RenderManager
{
    ...
    public void renderWitherSkull(Entity entityIn, float partialTicks)
    {
        ...
        {
        	int i = Minecraft.getMinecraft().gameSettings.ambientOcclusion == LightingQuality.HIGH.ordinal()
        			? EntityLighter.getBlendedLight(entityIn, partialTicks)
        					: entityIn.getBrightnessForRender(partialTicks);
            int j = i % 65536;
            int k = i / 65536;
            OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, (float)j / 1.0F, (float)k / 1.0F);
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
