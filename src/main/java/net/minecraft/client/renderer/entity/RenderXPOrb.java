...
public class RenderXPOrb extends Render<EntityXPOrb> {
	...
	public void doRender(EntityXPOrb entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		worldrenderer.pos(0.0F - f5, 0.0F - f6, 0.0D).tex(f, f3).color(k1, l1, i2, 128).normal(0.0F, 1.0F, 0.0F)
				.endVertex();
		worldrenderer.pos(f4 - f5, 0.0F - f6, 0.0D).tex(f1, f3).color(k1, l1, i2, 128).normal(0.0F, 1.0F, 0.0F)
				.endVertex();
		worldrenderer.pos(f4 - f5, 1.0F - f6, 0.0D).tex(f1, f2).color(k1, l1, i2, 128).normal(0.0F, 1.0F, 0.0F)
				.endVertex();
		worldrenderer.pos(0.0F - f5, 1.0F - f6, 0.0D).tex(f, f2).color(k1, l1, i2, 128).normal(0.0F, 1.0F, 0.0F)
				.endVertex();
		tessellator.draw();
