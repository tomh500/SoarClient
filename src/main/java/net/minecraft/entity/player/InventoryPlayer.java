...
public class InventoryPlayer implements IInventory {
	...
	private int getInventorySlotContainItemAndDamage(Item itemIn, int metadataIn) {
		for (int i = 0; i < this.mainInventory.length; ++i) {
			if (this.mainInventory[i] != null && this.mainInventory[i].getItem() == itemIn
					&& this.mainInventory[i].getMetadata() == metadataIn) {
				return i;
			}
		}
	...
	public void setCurrentItem(Item itemIn, int metadataIn, boolean isMetaSpecific, boolean p_146030_4_) {
		...
		int i = isMetaSpecific ? this.getInventorySlotContainItemAndDamage(itemIn, metadataIn)
	 ...
	 * 
	 * @param direction Direction to switch (1, 0, -1). 1 (any > 0) to select item
	 *                  left of current (decreasing currentItem index), -1 (any < 0)
	 *                  to select item right of current (increasing currentItem
	 *                  index). 0 has no effect.
	 */
