...
public class EntityAIFindEntityNearest extends EntityAIBase {
	...
	public EntityAIFindEntityNearest(EntityLiving mobIn, Class<? extends EntityLivingBase> p_i45884_2_) {
		...
		this.field_179443_c = new Predicate<EntityLivingBase>() {
			public boolean apply(EntityLivingBase p_apply_1_) {
				...
				return !p_apply_1_.isInvisible() && (!((double) p_apply_1_.getDistanceToEntity(EntityAIFindEntityNearest.this.mob) > d0) && EntityAITarget.isSuitableTarget(EntityAIFindEntityNearest.this.mob, p_apply_1_, false,
                        true));
			}
		};
	...
	public boolean shouldExecute() {
		...
		List<EntityLivingBase> list = this.mob.worldObj.getEntitiesWithinAABB(this.field_179439_f,
				this.mob.getEntityBoundingBox().expand(d0, 4.0D, d0), this.field_179443_c);
	...
	public boolean continueExecuting() {
		...
		if (entitylivingbase == null) {
			...
			double d0 = this.getFollowRange();
			return !(this.mob.getDistanceSqToEntity(entitylivingbase) > d0 * d0) && (!(entitylivingbase instanceof EntityPlayerMP)
