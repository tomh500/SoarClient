...
public class OptionPageFrame extends AbstractFrame {
    ...
    private void renderOptionTooltip(ControlElement<?> element) {
        ...
        final List<String> tooltip = new ArrayList<>(fontRenderer.listFormattedStringToWidth(option.getTooltip().getFormattedText(), boxWidth - (textPadding * 2)));

        final OptionImpact impact = option.getImpact();

        if (impact != null) {
            tooltip.add(EnumChatFormatting.GRAY + I18n.format("sodium.options.performance_impact_string", impact.getLocalizedName().getFormattedText()));
        }
