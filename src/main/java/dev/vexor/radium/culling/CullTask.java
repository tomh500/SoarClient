	...
	public void run() {
		while (client.isRunning()) {
			try {
				...
				if (RadiumEntityCulling.enabled && client.theWorld != null && client.thePlayer != null && client.thePlayer.ticksExisted > 10 && client.getRenderViewEntity() != null) {
					...
					if (requestCull || !(cameraMC.xCoord == lastPos.x && cameraMC.yCoord == lastPos.y && cameraMC.zCoord == lastPos.z)) {
						...
						while (iterable.hasNext()) {
							...
							if (!cullable.isForcedVisible()) {
							    ...
							    if(entity.getPositionVector().squareDistanceTo(cameraMC) > SodiumClientMod.options().culling.tracingDistance * SodiumClientMod.options().culling.tracingDistance) {
