...
public class CullTask implements Runnable {
	...
	public void run() {
		while (client.isRunning()) {
			try {
				...
				if (SoariumEntityCulling.enabled && client.theWorld != null && client.thePlayer != null && client.thePlayer.ticksExisted > 10 && client.getRenderViewEntity() != null) {
					...
					if (requestCull || !(cameraMC.xCoord == lastPos.x && cameraMC.yCoord == lastPos.y && cameraMC.zCoord == lastPos.z)) {
						...
						while(iterator.hasNext()) {
							...
							if (!cullable.isForcedVisible()) {
								...
								if(pos.distanceSq(new Vec3i(cameraMC.xCoord, cameraMC.yCoord, cameraMC.zCoord)) < 64 * 64) { // 64 is the fixed max tile view distance
						...
						while (iterable.hasNext()) {
							...
							if (!cullable.isForcedVisible()) {
							    ...
							    if(entity.getPositionVector().squareDistanceTo(cameraMC) > Soarium.getConfig().culling.tracingDistance * Soarium.getConfig().culling.tracingDistance) {
	...
	private boolean isSkippableArmorstand(Entity entity) {
	    if(!Soarium.getConfig().culling.skipMarkerArmorStands) return false;
