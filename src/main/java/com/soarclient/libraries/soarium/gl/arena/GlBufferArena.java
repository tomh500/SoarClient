package com.soarclient.libraries.soarium.gl.arena;
...
import com.soarclient.libraries.soarium.gl.arena.staging.StagingBuffer;
...
public class GlBufferArena {
	...
	private List<PendingBufferCopyCommand> buildTransferList(List<GlBufferSegment> usedSegments, long base) {
		...
		for (int i = 0; i < usedSegments.size(); i++) {
			...
			if (currentCopyCommand == null
					|| currentCopyCommand.getReadOffset() + currentCopyCommand.getLength() != s.getOffset()) {
				if (currentCopyCommand != null) {
	...
	private void transferSegments(CommandList commandList, Collection<PendingBufferCopyCommand> list, long capacity) {
		...
		for (PendingBufferCopyCommand cmd : list) {
			commandList.copyBufferSubData(srcBufferObj, dstBufferObj, cmd.getReadOffset() * this.stride,
					cmd.getWriteOffset() * this.stride, cmd.getLength() * this.stride);
		}
	...
	public boolean upload(CommandList commandList, Stream<PendingUpload> stream) {
		...
		// If the arena needs to re-allocate a buffer, this will allow us to check and
		// return an appropriate flag
		GlBuffer buffer = this.arenaBuffer;

		// A linked list is used as we'll be randomly removing elements and want O(1)
		// performance
		...
		// If we weren't able to upload some buffers, they will have been left behind in
		// the queue
		if (!queue.isEmpty()) {
			...
			// This will force a re-allocation and compaction, which will leave us a
			// continuous free segment
	...
	public void ensureCapacity(CommandList commandList, long elementCount) {
		// Re-sizing the arena results in a compaction, so any free space in the arena
		// will be
		// made into one contiguous segment, joined with the new segment of free space
		// we're asking for
		// We calculate the number of free elements in our arena and then subtract that
		// from the total requested
		long elementsNeeded = elementCount - (this.capacity - this.used);

		// Try to allocate some extra buffer space unless this is an unusually large
		// allocation
		...
		while (seg != null) {
			...
			if (next != null) {
				if (next.getOffset() < seg.getEnd()) {
					throw new IllegalStateException(
							"segment.next.start < segment.end: overlapping segments (corrupted)");
				} else if (next.getOffset() > seg.getEnd()) {
					throw new IllegalStateException(
							"segment.next.start > segment.end: not truly connected (sparsity error)");
				}

				if (next.isFree() && next.getNext() != null) {
					if (next.getNext().isFree()) {
						throw new IllegalStateException(
								"segment.free && segment.next.free: not merged consecutive segments");
					}
				}
			}
			...
			if (prev != null) {
				if (prev.getEnd() > seg.getOffset()) {
					throw new IllegalStateException(
							"segment.prev.end > segment.start: overlapping segments (corrupted)");
				} else if (prev.getEnd() < seg.getOffset()) {
					throw new IllegalStateException(
							"segment.prev.end < segment.start: not truly connected (sparsity error)");
				}

				if (prev.isFree() && prev.getPrev() != null) {
					if (prev.getPrev().isFree()) {
						throw new IllegalStateException(
								"segment.free && segment.prev.free: not merged consecutive segments");
					}
				}
			}
