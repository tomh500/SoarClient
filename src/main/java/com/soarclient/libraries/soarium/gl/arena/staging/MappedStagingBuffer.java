...
import com.soarclient.libraries.soarium.gl.buffer.*;
...
import com.soarclient.libraries.soarium.gl.functions.BufferStorageFunctions;
...
public class MappedStagingBuffer implements StagingBuffer {
	private static final EnumBitField<GlBufferStorageFlags> STORAGE_FLAGS = EnumBitField
			.of(GlBufferStorageFlags.PERSISTENT, GlBufferStorageFlags.CLIENT_STORAGE, GlBufferStorageFlags.MAP_WRITE);

	private static final EnumBitField<GlBufferMapFlags> MAP_FLAGS = EnumBitField.of(GlBufferMapFlags.PERSISTENT,
			GlBufferMapFlags.INVALIDATE_BUFFER, GlBufferMapFlags.WRITE, GlBufferMapFlags.EXPLICIT_FLUSH);
	...
	public void enqueueCopy(CommandList commandList, ByteBuffer data, GlBuffer dst, long writeOffset) {
		...
		// Split the transfer in two if we have enough available memory at the end and
		// start of the buffer
		if (length > remaining) {
	...
	public void flush(CommandList commandList) {
		...
		for (CopyCommand command : consolidateCopies(this.pendingCopies)) {
			...
			commandList.copyBufferSubData(this.mappedBuffer.buffer, command.buffer, command.readOffset,
					command.writeOffset, command.bytes);
		}
	...
	private static List<CopyCommand> consolidateCopies(PriorityQueue<CopyCommand> queue) {
		...
		while (!queue.isEmpty()) {
			...
			if (last != null) {
				if (last.buffer == command.buffer && last.writeOffset + last.bytes == command.writeOffset
