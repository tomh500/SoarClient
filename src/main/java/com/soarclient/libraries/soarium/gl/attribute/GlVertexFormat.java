package com.soarclient.libraries.soarium.gl.attribute;
...
/**
 * Provides a generic vertex format which contains the attributes defined by a
 * {@link VertexFormatAttribute}. Other code can then retrieve the attributes
 * and work with encoded data in a generic manner without needing to rely on a
 * specific format.
 */
public class GlVertexFormat {
	...
	public GlVertexFormat(Map<VertexFormatAttribute, GlVertexAttribute> attributesKeyed,
			GlVertexAttributeBinding[] bindings, int stride) {
	...
	/**
	 * Returns the {@link GlVertexAttribute} of this vertex format bound to the type
	 * {@param name}.
	 * 
	...
	public static class Builder {
		...
		public Builder addElement(VertexFormatAttribute attribute, int binding, int pointer) {
			return this.addElement(attribute, binding, new GlVertexAttribute(attribute.format(), attribute.count(),
					attribute.normalized(), pointer, this.stride, attribute.intType()));
		}

		/**
		 * Adds an vertex attribute which will be bound to the given generic attribute
		 * type.
		 *
		 * @param type      The generic attribute type
		 * @param attribute The attribute to bind
		 * @throws IllegalStateException If an attribute is already bound to the generic
		 *                               type
		 */
		...
		public GlVertexFormat build() {
			...
			// The stride must be large enough to cover all attributes. This still allows
