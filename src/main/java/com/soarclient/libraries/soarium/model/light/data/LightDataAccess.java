...
/**
 * The light data cache is used to make accessing the light data and occlusion
 * properties of blocks cheaper. The data for each block is stored as a long
 * integer with packed fields in order to work around the lack of value types in
 * Java.
 *
 * This code is not very pretty, but it does perform significantly faster than
 * the vanilla implementation and has good cache locality.
 *
 * Each long integer contains the following fields: - OP: Block opacity test,
 * true if opaque - FO: Full block opaque test, true if opaque - AO: Ambient
 * occlusion, floating point value in the range of 0.0..1.0 encoded as an 12-bit
 * unsigned integer - LM: Light map texture coordinates, two packed UV shorts in
 * an integer
 *
 * You can use the various static pack/unpack methods to extract these values in
 * a usable format.
 */
public abstract class LightDataAccess {
	...
	/**
	 * Returns the light data for the block at the given position. The property
	 * fields can then be accessed using the various unpack methods below.
	 */
	...
	protected long compute(int x, int y, int z) {
		...
		// FIX: Calculate lightmap data for light-emitting or emissive blocks, even
		// though they are full and opaque.
