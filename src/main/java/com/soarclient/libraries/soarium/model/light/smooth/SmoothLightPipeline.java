package com.soarclient.libraries.soarium.model.light.smooth;
...
/**
 * A light pipeline which produces smooth interpolated lighting and ambient
 * occlusion for model quads. This implementation makes a number of improvements
 * over vanilla's own "smooth lighting" option. In no particular order:
 *
 * - Ambient occlusion of block slopes underwater no longer produces broken
 * results (fixes MC-149211) - Smooth lighting now works when underwater (fixes
 * MC-68129) - Corner blocks are now selected from the correct set of neighbors
 * above block faces (fixes MC-148689 and MC-12558) - Shading issues caused by
 * anisotropy are fixed by re-orientating quads to a consistent ordering (fixes
 * MC-136302) - Inset block faces are correctly shaded by their neighbors,
 * fixing a number of problems with non-full blocks such as grass paths (fixes
 * MC-11783 and MC-108621) - Synchronization issues between the main render
 * thread's light engine and chunk build worker threads are corrected by copying
 * light data alongside block states, fixing a number of inconsistencies in
 * baked chunks (no open issue)
 *
 * This implementation also includes a significant number of optimizations:
 *
 * - Computed light data for a given block face is cached and re-used again when
 * multiple quads exist for a given facing, making complex block models less
 * expensive to render - The light data cache encodes as much information as
 * possible into integer words to improve cache locality and to eliminate the
 * multiple array lookups that would otherwise be needed, significantly speeding
 * up this section - Block faces aligned to the block grid use a fast-path for
 * mapping corner light values to vertices without expensive interpolation or
 * blending, speeding up most block renders - Some critical code paths have been
 * re-written to hit the JVM's happy path, allowing it to perform
 * auto-vectorization of the blend functions - Information about a given model
 * quad is cached to enable the light pipeline to make certain assumptions and
...
public class SmoothLightPipeline implements LightPipeline {
	...
	/**
	 * A temporary array for storing the intermediary results of weight data for
	 * non-aligned face blending.
	 */
	...
	public void calculate(ModelQuadView quad, BlockPos pos, QuadLightData out, EnumFacing cullFace, EnumFacing face,
		...
		// If the model quad is aligned to the block's face and covers it entirely, we
		// can take a fast path and directly
		// map the corner values onto this quad's vertices. This covers most situations
		// during rendering and provides
		// a modest speed-up.
		// To match vanilla behavior, also treat the face as aligned if it is parallel
		// and the block state is a full cube
	...
	/**
	 * Quickly calculates the light data for a full grid-aligned quad. This
	 * represents the most common case (outward facing quads on a full-block model)
	 * and avoids interpolation between neighbors as each corner will only ever have
	 * two contributing sides. Flags: IS_ALIGNED, !IS_PARTIAL
	 */
	...
	/**
	 * Calculates the light data for a grid-aligned quad that does not cover the
	 * entire block volume's face. Flags: IS_ALIGNED, IS_PARTIAL
	 */
	private void applyAlignedPartialFace(AoNeighborInfo neighborInfo, ModelQuadView quad, BlockPos pos, EnumFacing dir,
			QuadLightData out) {
		for (int i = 0; i < 4; i++) {
			// Clamp the vertex positions to the block's boundaries to prevent weird errors
			// in lighting
	 ...
	 * {@link #applyNonParallelFace(AoNeighborInfo, ModelQuadView, BlockPos, EnumFacing, QuadLightData)}
	 * but with the check for a depth of approximately 0 removed. If the quad is
	 * parallel but not aligned, all of its vertices will have the same depth and
	 * this depth must be approximately greater than 0, meaning the check for 0 will
	...
	private void applyParallelFace(AoNeighborInfo neighborInfo, ModelQuadView quad, BlockPos pos, EnumFacing dir,
		...
		for (int i = 0; i < 4; i++) {
			...
			// If the quad is approximately grid-aligned (not inset) to the other side of
			// the block, avoid unnecessary
			...
			if (MathHelper.epsilonEquals(depth, 1.0F)) {
				...
				// Blend the occlusion factor between the blocks directly beside this face and
				// the blocks above it
	...
	private void applyNonParallelFace(AoNeighborInfo neighborInfo, ModelQuadView quad, BlockPos pos, EnumFacing dir,
		...
		for (int i = 0; i < 4; i++) {
			...
			// If the quad is approximately grid-aligned (not inset), avoid unnecessary
			// computation by treating it is as aligned
	...
	/**
	 * Returns the cached data for a given facing or calculates it if it hasn't been
	 * cached.
	 */
	...
	/**
	 * Returns a texture coordinate on the light map texture for the given block and
	 * sky light values.
	 */
