package com.soarclient.libraries.soarium.model.light.flat;
...
/**
 * A light pipeline which implements "classic-style" lighting through simply
 * using the light value of the adjacent block to a face.
 */
public class FlatLightPipeline implements LightPipeline {
	...
	public void calculate(ModelQuadView quad, BlockPos pos, QuadLightData out, EnumFacing cullFace, EnumFacing face,
		...
		if (cullFace != null) {
			...
			if ((flags & ModelQuadFlags.IS_ALIGNED) != 0 || ((flags & ModelQuadFlags.IS_PARALLEL) != 0
	...
	/**
	 * When vanilla computes an offset lightmap with flat lighting, it passes the
	 * original BlockState but the offset BlockPos to
	 * {@link WorldRenderer#getLightmapCoordinates(BlockRenderView, BlockState, BlockPos)}.
	 * This does not make much sense but fixes certain issues, primarily dark quads
	 * on light-emitting blocks behind tinted glass. {@link LightDataAccess} cannot
	 * efficiently store lightmaps computed with inconsistent values so this method
	 * exists to mirror vanilla behavior as closely as possible.
	 */
