...
/**
 * Provides some utilities for packing and unpacking color components from
 * packed integer colors in ABGR format, which is used by OpenGL for color
 * vectors.
 *
 * | 32 | 24 | 16 | 8 | | 0110 1100 | 0110 1100 | 0110 1100 | 0110 1100 | |
...
public class ColorABGR implements ColorU8 {
	...
	/**
	 * Packs the specified color components into ABGR format. The alpha component is
	 * fully opaque.
	 * 
	...
	public static int pack(int r, int g, int b, int a) {
		return ((a & COMPONENT_MASK) << ALPHA_COMPONENT_OFFSET) | ((b & COMPONENT_MASK) << BLUE_COMPONENT_OFFSET)
				| ((g & COMPONENT_MASK) << GREEN_COMPONENT_OFFSET) | ((r & COMPONENT_MASK) << RED_COMPONENT_OFFSET);
	}
	...
	/**
	 * Converts the color components from normalized floating point into a packed
	 * integer in ABGR format.
	 * 
	 * @see ColorABGR#pack(int, int, int, int)
	 */
	public static int pack(float r, float g, float b, float a) {
		return pack(ColorU8.normalizedFloatToByte(r), ColorU8.normalizedFloatToByte(g),
				ColorU8.normalizedFloatToByte(b), ColorU8.normalizedFloatToByte(a));
	}
	...
	/**
	 * Multiplies the RGB components of the color with the provided factor. The
	...
	/**
	 * See {@link #mulRGB(int, int)}. This function is identical, but it accepts a
	 * float in [0.0, 1.0] instead, which is then mapped to [0, 255].
	 *
	 * @param color  The packed 32-bit ABGR color to be multiplied
	 * @param factor The darkening factor (in the range of 0.0..1.0) to multiply
	 *               with
	 */
	...
	/**
	 * Shuffles the ordering of the ABGR color so that it can then be written out to
	 * memory in the platform's native byte ordering. This should be used when
	 * writing the packed color to memory (in native-order) as a 32-bit word.
	 */
	...
	/**
	 * Shuffles the ordering of the ABGR color from the platform's native byte
	 * ordering. This should be used when reading the packed color from memory (in
	 * native-order) as a 32-bit word.
	 */
