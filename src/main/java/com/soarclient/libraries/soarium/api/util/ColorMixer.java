package com.soarclient.libraries.soarium.api.util;

import org.jetbrains.annotations.ApiStatus;

/**
 * A collection of optimized color mixing functions which directly operate on
 * packed color values. These functions are agnostic to the ordering of color
 * channels, and the output value will always use the same channel ordering as
 * the input values.
 */
public class ColorMixer {
	/**
	 * <p>
	 * Linearly interpolate between the {@param start} and {@param end} points,
	 * represented as packed unsigned 8-bit values within a 32-bit integer. The
	 ...
	 *
	 * <p>
	 * The results are undefined if {@param weight} is not within the interval [0,
	 * 255].
	 * </p>
	 *
	 * <p>
	 * If {@param start} and {@param end} are the same value, the result of this
	 * function will always be that value, regardless of {@param weight}.
	 * </p>
	 *
	 * @param start  The value at the start of the range to interpolate
	 * @param end    The value at the end of the range to interpolate
	 * @param weight The weight value used to interpolate between color values (in
	 *               0..255 range)
	...
	public static int mix(int start, int end, int weight) {
		// De-interleave the 8-bit component lanes into high and low halves for each
		// point.
		// Multiply the start point by alpha, and the end point by 1-alpha, to produce
		// Q8.8 fixed-point intermediates.
		...
		// Round the fixed-point values to the nearest integer, and interleave the high
		// and low halves to
		// produce the final packed result.
		final long result = (((hi + 0x00FF00FFL) >>> 8) & 0x00FF00FFL) | (((lo + 0xFF00FF00L) >>> 8) & 0xFF00FF00L);
	...
	/**
	 * <p>
	 * This function is identical to {@link ColorMixer#mix(int, int, int)}, but
	 * {@param weight} is a normalized floating-point value within the interval of
	 * [0.0, 1.0].
	 * </p>
	 *
	 * <p>
	 * The results are undefined if {@param weight} is not within the interval [0.0,
	 * 1.0].
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Performs bi-linear interpolation on a 2x2 matrix of color values to derive
	 * the point (x, y). This is more efficient than chaining
	 * {@link #mul(int, float)} calls.
	 * </p>
	 *
	 * <p>
	 * The results are undefined if {@param x} and {@param y} are not within the
	 * interval [0.0, 1.0].
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Multiplies the packed 8-bit values component-wise to produce 16-bit
	 * intermediaries, and then round to the nearest 8-bit representation (similar
	 * to floating-point.)
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Multiplies each 8-bit component against the factor to produce 16-bit
	 * intermediaries, and then round to the nearest 8-bit representation (similar
	 * to floating-point.)
	 * </p>
	 *
	 * <p>
	 * The results are undefined if {@param factor} is not within the interval [0,
	 * 255].
	 * </p>
	 *
	...
	public static int mul(int color, int factor) {
		...
		// Round the Q8.8 fixed-point values to the nearest integer, and interleave the
		// high and low halves to
	...
	/**
	 * See {@link #mul(int, int)}, which this function is identical to, except that
	 * it takes a floating point value in the interval of [0.0, 1.0] and maps it to
	 * [0, 255].
	 *
	 * <p>
	 * The results are undefined if {@param factor} is not within the interval [0.0,
	 * 1.0].
	 * </p>
	 */
