...
public class ChunkBuilder {
	...
	/**
	 * Returns the remaining effort for tasks which should be scheduled this frame.
	 * If an attempt is made to spawn more tasks than the budget allows, it will
	...
	/**
	 * <p>
	 * Notifies all worker threads to stop and blocks until all workers terminate.
	 * After the workers have been shut down, all tasks are cancelled and the
	 * pending queues are cleared. If the builder is already stopped, this method
	 * does nothing and exits.
	 * </p>
	 *
	 * <p>
	 * After shutdown, all previously scheduled jobs will have been cancelled. Jobs
	 * that finished while waiting for worker threads to shut down will still have
	 * their results processed for later cleanup.
	 * </p>
	 */
	...
	/**
	 * Returns the "optimal" number of threads to be used for chunk build tasks.
	 * This will always return at least one thread.
	 */
	...
	private static int getThreadCount() {
		int requested = Soarium.getConfig().performance.chunkBuilderThreads;
	...
	private class WorkerRunnable implements Runnable {
		// Making this thread-local provides a small boost to performance by avoiding
