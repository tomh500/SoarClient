...
import com.soarclient.libraries.soarium.util.collections.BitArray;
...
public class TopoGraphSorting {
	 ...
	 * inside.
	 * <p>
	 * A small epsilon is added in the test to account for floating point errors,
	 * making it harder for a point on the edge to be considered inside.
	 *
	 * @param planeDistance dot product of the plane
	 * @param planeNormal   the normal of the plane
	 ...
	 */
	private static boolean pointInsideHalfSpaceEpsilon(float planeDistance, Vector3fc planeNormal, float x, float y,
			float z) {
		return planeNormal.dot(x, y, z) + HALF_SPACE_EPSILON < planeDistance;
	}

	/**
	 * Test if the given point is outside the half space defined by the plane anchor
	 * and the plane normal. The normal points away from the space considered to be
	 * inside.
	 * <p>
	 * A small epsilon is subtracted in the test to account for floating point
	 * errors, making it harder for a point on the edge to be considered outside.
	 *
	 * @param planeDistance dot product of the plane
	 * @param planeNormal   the normal of the plane
	 * @param x             x coordinate of the point
	 ...
	 * @return true if the point is inside the half space
	 */
	private static boolean pointOutsideHalfSpaceEpsilon(float planeDistance, Vector3fc planeNormal, float x, float y,
			float z) {
		return planeNormal.dot(x, y, z) - HALF_SPACE_EPSILON > planeDistance;
	}
	...
	public static boolean orthogonalQuadVisibleThrough(TQuad quadA, TQuad quadB) {
		...
		// test that B has an extent within A's half space and that A is not fully
		// within B's half space
		...
		// if they're visible and their bounding boxes intersect and apply a heuristic
		// to resolve
	...
	private static boolean visibilityWithSeparator(TQuad quadA, TQuad quadB,
		...
		// hard problem and this is an approximation. The fully correct topo sort would
		// need
	...
	private static boolean quadVisibleThrough(TQuad quad, TQuad other,
		...
		if (quadFacing != ModelQuadFacing.UNASSIGNED && otherFacing != ModelQuadFacing.UNASSIGNED) {
			...
			// at least one of the other quad's vertexes must be inside the half space of
			// the first quad
			var otherInsideQuad = false;
			for (int i = 0, itemIndex = 0; i < 4; i++) {
				if (pointInsideHalfSpaceEpsilon(quadDot, quadNormal, otherVertexPositions[itemIndex++],
						otherVertexPositions[itemIndex++], otherVertexPositions[itemIndex++])) {
			...
			if (otherInsideQuad) {
				...
				for (int i = 0, itemIndex = 0; i < 4; i++) {
					if (pointOutsideHalfSpaceEpsilon(otherDot, otherNormal, quadVertexPositions[itemIndex++],
							quadVertexPositions[itemIndex++], quadVertexPositions[itemIndex++])) {
	 ...
	 *                          visibility check
	 */
	public static boolean topoGraphSort(IntConsumer indexConsumer, TQuad[] allQuads,
			Object2ReferenceOpenHashMap<Vector3fc, float[]> distancesByNormal, Vector3fc cameraPos) {
		// if enabled, check for visibility and produce a mapping of indices
		TQuad[] quads;
		int[] activeToRealIndex = null;

		// keep track of the number of quads to be processed, this is possibly less than
		// quads.length
		int quadCount = 0;
	...
	public static boolean topoGraphSort(IntConsumer indexConsumer, TQuad[] quads, int quadCount,
			int[] activeToRealIndex, Object2ReferenceOpenHashMap<Vector3fc, float[]> distancesByNormal,
			Vector3fc cameraPos) {
