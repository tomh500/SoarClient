...
public class TQuad {
	...
	private TQuad(ModelQuadFacing facing, float[] extents, float[] vertexPositions, Vector3fc center,
			int packedNormal) {
		this.facing = facing;
	...
	static TQuad fromAligned(ModelQuadFacing facing, float[] extents, float[] vertexPositions, Vector3fc center) {
		return new TQuad(facing, extents, vertexPositions, center,
				ModelQuadFacing.PACKED_ALIGNED_NORMALS[facing.ordinal()]);
	}

	static TQuad fromUnaligned(ModelQuadFacing facing, float[] extents, float[] vertexPositions, Vector3fc center,
			int packedNormal) {
	...
	/**
	 * Calculates the facing of the quad based on the quantized normal. This updates
	 * the dot product to be consistent with the new facing. Since this method
	 * computed and allocates the quantized normal, it should be used sparingly and
	 * only when the quantized normal is calculated anyway. Additionally, it can
	 * modify the facing and not product of the quad which the caller should be
	 * aware of.
	 *
	...
	public ModelQuadFacing useQuantizedFacing() {
		if (!this.facing.isAligned()) {
			...
			this.facing = ModelQuadFacing.fromNormal(this.quantizedNormal.x(), this.quantizedNormal.y(),
					this.quantizedNormal.z());
			if (this.facing.isAligned()) {
	...
	public float[] getVertexPositions() {
		...
		// (we don't want to be preemptively collecting vertex positions for all aligned
		// quads)
	...
	public Vector3fc getCenter() {
		...
		if (this.center == null) {
			this.center = new Vector3f((this.extents[0] + this.extents[3]) / 2, (this.extents[1] + this.extents[4]) / 2,
	...
	public Vector3fc getAccurateNormal() {
		if (this.facing.isAligned()) {
			...
			if (this.accurateNormal == null) {
				this.accurateNormal = new Vector3f(NormI8.unpackX(this.packedNormal), NormI8.unpackY(this.packedNormal),
	...
	private void computeQuantizedNormal() {
		...
		var normal = new Vector3f((int) (normX * QUANTIZATION_FACTOR), (int) (normY * QUANTIZATION_FACTOR),
