package com.soarclient.libraries.soarium.render.chunk.translucent_sorting.bsp_tree;
...
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.TQuad;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.TranslucentGeometryCollector;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.data.TopoGraphSorting;
import com.soarclient.libraries.soarium.util.MathUtil;
import com.soarclient.libraries.soarium.util.sorting.RadixSort;
 ...
 * <p>
 * Implementation notes: - Presorting the points in block-sized buckets doesn't
 * help. It seems the sort algorithm is just fast enough to handle this. -
 * Eliminating the use of partition objects doesn't help. Since there's usually
 * just very few partitions, it's not worth it, it seems. - Using fastutil's
 * LongArrays sorting options (radix and quicksort) is slower than using
 * Arrays.sort (which uses DualPivotQuicksort internally), even on worlds with
 * player-built structures. - A simple attempt at lazily writing index data to
 * the buffer didn't yield a performance improvement. Maybe applying it to the
 * multi partition node would be more effective (but also much more complex and
 * slower).
 * <p>
 ...
 * <a href="https://stackoverflow.com/q/43299299">StackOverflow</a>
 * <p>
 ...
 * it either requires unaligned partitioning (a hard problem not solved here) or
 * it's unpartitionable. It would be possible to insert a topo sorting node
...
abstract class InnerPartitionBSPNode extends BSPNode {
	...
	static NodeReuseData prepareNodeReuse(BSPWorkspace workspace, IntArrayList indexes, int depth) {
		...
		if (workspace.prepareNodeReuse && depth == 1 && indexes.size() > NODE_REUSE_THRESHOLD) {
			...
			return new NodeReuseData(quadExtents, BSPSortState.compressIndexes(indexes, false), indexes.size(),
					maxIndex);
		}
		return null;
	}
	...
	static InnerPartitionBSPNode attemptNodeReuse(BSPWorkspace workspace, IntArrayList newIndexes,
	...
	/**
	 * Encoding with {@link MathUtil#floatToComparableInt(float)} is necessary here
	 * to ensure negative distances are not sorted backwards. Simply converting
	 * potentially negative floats to int bits using
	 * {@link Float#floatToRawIntBits(float)} would sort negative floats backwards
	 * amongst themselves.
	 * <p>
	 * Note that negative floats convert to negative integers with this method which
	 * is ok, since it yields an overall negative long that gets sorted correctly
	 * before the longs that encode positive floats as distances.
	 */
	...
	static BSPNode build(BSPWorkspace workspace, IntArrayList indexes, int depth, BSPNode oldNode) {
		...
		for (int axisCount = 0; axisCount < 3; axisCount++) {
			...
			// simplified SNR heuristic as seen in
			// TranslucentGeometryCollector#sortTypeHeuristic (case D)
			if (!ModelQuadFacing.bitmapHasUnassigned(alignedFacingBitmap)) {
				int alignedNormalCount = Integer.bitCount(alignedFacingBitmap);
				if (alignedNormalCount == 1
						|| alignedNormalCount == 2 && ModelQuadFacing.bitmapIsOpposingAligned(alignedFacingBitmap)) {
			...
			if (partitions.size() <= 2) {
				...
				if (outside == null || !endsWithPlane) {
					return InnerBinaryPartitionBSPNode.buildFromPartitions(workspace, indexes, depth, oldNode, inside,
							outside, axis);
				}
			}

			// create a multi-partition node
			return InnerMultiPartitionBSPNode.buildFromPartitions(workspace, indexes, depth, oldNode, partitions, axis,
					endsWithPlane);
		}
		...
		if (multiLeafNode == null) {
			throw new BSPBuildFailureException(
					"No partition found but not intersecting and can't be statically topo sorted");
		}
		return multiLeafNode;
	}

	static private BSPNode handleIntersecting(BSPWorkspace workspace, IntArrayList indexes, int depth,
			BSPNode oldNode) {
		...
		if (primaryIntersectorIndexes != null) {
			...
			return InnerFixedDoubleBSPNode.buildFromParts(workspace, indexes, depth, oldNode, nonPrimaryIntersectors,
					primaryIntersectorQuadIndexes);
		}
	...
	static private BSPNode buildTopoMultiLeafNode(BSPWorkspace workspace, IntArrayList indexes) {
		...
		// since it's being sorted statically and the sort order won't change based on
		// the camera position
	...
	static private BSPNode buildSNRLeafNodeFromQuads(BSPWorkspace workspace, IntArrayList indexes,
		...
		if (RadixSort.useRadixSort(indexes.size())) {
			...
			for (int i = 0; i < indexes.size(); i++) {
				var quadIndex = indexes.getInt(i);
				int dotProductComponent = MathUtil
						.floatToComparableInt(workspace.quads[quadIndex].getAccurateDotProduct());
	...
	static private BSPNode buildSNRLeafNodeFromPoints(BSPWorkspace workspace, LongArrayList points) {
		...
		// since the quads are aligned and are all INTERVAL_SIDE, there's no issues with
		// duplicates.
		...
		for (int i = 0; i < points.size(); i++) {
			...
			// this means forwards is written from the start and backwards is written from
			// the end
