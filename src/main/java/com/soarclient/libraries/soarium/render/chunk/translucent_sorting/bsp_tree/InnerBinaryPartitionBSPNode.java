...
class InnerBinaryPartitionBSPNode extends InnerPartitionBSPNode {
	...
	InnerBinaryPartitionBSPNode(NodeReuseData reuseData, float planeDistance, int axis, BSPNode inside, BSPNode outside,
			int[] onPlaneQuads) {
	...
	static BSPNode buildFromPartitions(BSPWorkspace workspace, IntArrayList indexes, int depth, BSPNode oldNode,
		...
		if (oldNode instanceof InnerBinaryPartitionBSPNode binaryNode && binaryNode.axis == axis
		...
		return new InnerBinaryPartitionBSPNode(prepareNodeReuse(workspace, indexes, depth), partitionDistance, axis,
