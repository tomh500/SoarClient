...
import com.soarclient.libraries.soarium.render.chunk.compile.buffers.BakedChunkModelBuilder;
...
/**
 * A collection of temporary buffers for each worker thread which will be used
 * to build chunk meshes for given render passes. This makes a best-effort
 * attempt to pick a suitable size for each scratch buffer, but will never try
 * to shrink a buffer.
 */
public class ChunkBuildBuffers {
	...
	/**
	 * Creates immutable baked chunk meshes from all non-empty scratch buffers. This
	 * is used after all blocks have been rendered to pass the finished meshes over
	 * to the graphics card. This function can be called multiple times to return
	 * multiple copies.
	 */
