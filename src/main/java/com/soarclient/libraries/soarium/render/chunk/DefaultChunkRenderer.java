...
import com.soarclient.libraries.soarium.gl.device.DrawCommandList;
import com.soarclient.libraries.soarium.gl.device.MultiDrawBatch;
...
import com.soarclient.libraries.soarium.render.chunk.data.SectionRenderDataStorage;
import com.soarclient.libraries.soarium.render.chunk.data.SectionRenderDataUnsafe;
import com.soarclient.libraries.soarium.render.chunk.lists.ChunkRenderList;
import com.soarclient.libraries.soarium.render.chunk.lists.ChunkRenderListIterable;
import com.soarclient.libraries.soarium.render.chunk.region.RenderRegion;
import com.soarclient.libraries.soarium.render.chunk.shader.ChunkShaderInterface;
...
import com.soarclient.libraries.soarium.util.BitwiseMath;
...
public class DefaultChunkRenderer extends ShaderChunkRenderer {
	...
	public DefaultChunkRenderer(RenderDevice device, ChunkVertexType vertexType) {
		...
		this.sharedIndexBuffer = new SharedQuadIndexBuffer(device.createCommandList(),
				SharedQuadIndexBuffer.IndexType.INTEGER);
	}
	...
	public void render(ChunkRenderMatrices matrices, CommandList commandList, ChunkRenderListIterable renderLists,
		...
		final boolean useBlockFaceCulling = Soarium.getConfig().performance.useBlockFaceCulling;
		...
		while (iterator.hasNext()) {
			...
			// When the shared index buffer is being used, we must ensure the storage has
	...
	private static boolean isTranslucentRenderPass(TerrainRenderPass renderPass) {
		return renderPass.isTranslucent() && Soarium.getConfig().performance.getSortBehavior() != SortBehavior.OFF;
	}

	private static void fillCommandBuffer(MultiDrawBatch batch, RenderRegion renderRegion,
		...
		while (iterator.hasNext()) {
			...
			// If there are no geometry sets to render, don't try to build a draw command
			// buffer for this section
			if (slices == 0) {
				continue;
			}
	...
	private static void addNonIndexedDrawCommands(MultiDrawBatch batch, long pMeshData, int mask) {
		...
		for (int facing = 0; facing < ModelQuadFacing.COUNT; facing++) {
			...
			MemoryUtil.memPutInt(pBaseVertex + (size << 2),
					(int) SectionRenderDataUnsafe.getVertexOffset(pMeshData, facing));
			MemoryUtil.memPutInt(pElementCount + (size << 2),
					(int) SectionRenderDataUnsafe.getElementCount(pMeshData, facing));
			MemoryUtil.memPutAddress(pElementPointer + (size << Pointer.POINTER_SHIFT),
					0 /* using a shared index buffer */);
	...
	/**
	 * Generates the draw commands for a chunk's meshes, where each mesh has a
	...
	private static void addIndexedDrawCommands(MultiDrawBatch batch, long pMeshData, int mask) {
		...
		for (int facing = 0; facing < ModelQuadFacing.COUNT; facing++) {
			...
			// the section render data storage for the indices stores the offset in indices
			// (also called elements)
			MemoryUtil.memPutAddress(pElementPointer + (size << Pointer.POINTER_SHIFT), elementOffset << 2);

			// adding the number of elements works because the index data has one index per
			// element (which are the indices)
	...
	private static int getVisibleFaces(int originX, int originY, int originZ, int chunkX, int chunkY, int chunkZ) {
		...
		//
		// Normally, this would be a ridiculous way to handle the problem. But the
		// Hotspot VM's
		// heuristic for generating SETcc/CMOV instructions is broken, and it will
		// always create a
		...
		// (a > b) ? 1 : 0
		//
		// So we have to instead rely on sign-bit extension and masking (which generates
		// a ton
		...
		// (b - a) >> 31
		//
		// This works because if (a > b) then (b - a) will always create a negative
		// number. We then shift the sign bit
		// into the least significant bit's position (which also discards any bits
		// following the sign bit) to get the
		// output we are looking for.
		//
		// If you look at the output which LLVM produces for a series of ternaries, you
		// will instantly become distraught,
		// because it manages to a) correctly evaluate the cost of instructions, and b)
		// go so far
	...
	private static void setModelMatrixUniforms(ChunkShaderInterface shader, RenderRegion region,
	...
	private GlTessellation createRegionTessellation(CommandList commandList, RenderRegion.DeviceResources resources,
			boolean useSharedIndexBuffer) {
		return commandList.createTessellation(GlPrimitiveType.TRIANGLES, new TessellationBinding[] {
				TessellationBinding.forVertexBuffer(resources.getGeometryBuffer(),
						this.vertexFormat.getShaderBindings()),
				TessellationBinding.forElementBuffer(useSharedIndexBuffer ? this.sharedIndexBuffer.getBufferObject()
