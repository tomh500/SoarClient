...
public class ChunkJobCollector {
	...
	public boolean hasBudgetFor(int effort, boolean ignoreEffortCategory) {
		...
		if (ignoreEffortCategory) {
			return this.submittedLowEffort + this.submittedHighEffort + effort <= this.highEffortBudget
					+ this.lowEffortBudget;
		}
		return effort <= ChunkBuilder.LOW_EFFORT ? this.submittedLowEffort + effort <= this.lowEffortBudget
