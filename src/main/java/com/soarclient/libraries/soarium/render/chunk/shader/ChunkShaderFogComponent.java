...
import com.soarclient.libraries.soarium.gl.shader.uniform.GlUniformFloat;
import com.soarclient.libraries.soarium.gl.shader.uniform.GlUniformFloat4v;
import com.soarclient.libraries.soarium.gl.shader.uniform.GlUniformInt;

/**
 * These shader implementations try to remain compatible with the deprecated
 * fixed function pipeline by manually copying the state into each shader's
 * uniforms. The shader code itself is a straight-forward implementation of the
 * fog functions themselves from the fixed-function pipeline, except that they
 * use the distance from the camera rather than the z-buffer to produce better
 * looking fog that doesn't move with the player's view angle.
 *
 * Minecraft itself will actually try to enable distance-based fog by using the
 * proprietary NV_fog_distance extension, but as the name implies, this only
 * works on graphics cards produced by NVIDIA. The shader implementation however
 * does not depend on any vendor-specific extensions and is written using very
 * simple GLSL code.
 */
