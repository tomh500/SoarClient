...
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.bsp_tree.BSPNode;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.bsp_tree.BSPResult;
...
public class DynamicBSPData extends DynamicData {
	...
	private DynamicBSPData(SectionPos sectionPos, int vertexCount, BSPResult result, Vector3dc initialCameraPos,
			TQuad[] quads, int generation) {
	...
	public static DynamicBSPData fromMesh(int vertexCount, CombinedCameraPos cameraPos, TQuad[] quads,
			SectionPos sectionPos, TranslucentData oldData) {
		...
		var dynamicData = new DynamicBSPData(sectionPos, vertexCount, result, cameraPos.getAbsoluteCameraPos(), quads,
				generation);
