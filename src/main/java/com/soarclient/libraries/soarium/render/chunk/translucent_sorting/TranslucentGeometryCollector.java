package com.soarclient.libraries.soarium.render.chunk.translucent_sorting;
...
import com.soarclient.libraries.soarium.render.chunk.compile.pipeline.fluid.DefaultFluidRenderer;
import com.soarclient.libraries.soarium.render.chunk.data.BuiltSectionMeshParts;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.bsp_tree.BSPBuildFailureException;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.data.AnyOrderData;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.data.CombinedCameraPos;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.data.DynamicBSPData;
...
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.data.StaticNormalRelativeData;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.data.StaticTopoData;
 ...
 * three stages: 1. During meshing, it collects the geometry and calculates some
 * metrics on the fly. These are later used for the sort type heuristic. 2. With
 * {@link #finishRendering()} it finishes the geometry collection, generates the
 * quad list, and calculates additional metrics. Then the sort type is
 * determined with a heuristic based on the collected metrics. This determines
 * if block face culling can be enabled. - Now the {@link BuiltSectionMeshParts}
 * is generated, which yields the vertex counts. 3. The vertex counts and the
 * mesh parts object are used by the collector in the construction of the
 * {@link TranslucentData} object. The data object allocates memory for the
 * index data and performs the first (and for static sort types, only) sort. -
 ...
 * object is integrated into {@link SortTriggering} when main thread receives
 * the task result.
 */
public class TranslucentGeometryCollector {
	...
	private final float[] extents = new float[] { Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY,
	...
	private final float[] alignedExtremes = new float[] { Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY,
			Float.NEGATIVE_INFINITY, Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY };
	...
	public void appendQuad(int packedNormal, ChunkVertexEncoder.Vertex[] vertices, ModelQuadFacing facing) {
		...
		// check if we need to store vertex positions for this quad, only necessary if
		// it's unaligned or rotated (yet aligned)
		var needsVertexPositions = uniqueVertexes != 4 || !facing.isAligned();
		if (!needsVertexPositions) {
			for (int i = 0; i < 4; i++) {
				var vertex = vertices[i];
				if (vertex.x != posYExtent && vertex.x != negYExtent || vertex.y != posZExtent && vertex.y != negZExtent
	...
	private static SortType filterSortType(SortType sortType) {
		SortBehavior sortBehavior = Soarium.getConfig().performance.getSortBehavior();
		switch (sortBehavior) {
		case OFF:
	...
	public static final int STATIC_TOPO_UNKNOWN_FALLBACK_LIMIT = STATIC_TOPO_SORT_ATTEMPT_LIMITS[STATIC_TOPO_SORT_ATTEMPT_LIMITS.length
			- 1];
	 ...
	 * cases: <a href=
	 * "https://hackmd.io/@douira100/sodium-sl-gfni#Special-Sorting-Cases">...</a>
	 * <p>
	 ...
	 * then they can't be seen through each other and this section can be ignored.
	 * <p>
	 ...
	 * distances line up with the bounding box allows the exclusion of some sections
	 * containing a single convex translucent cuboid (of which not all faces need to
	 * exist).
	 * <p>
	 * D: If there are only two normals which are opposites of each other, then a
	 * special fixed sort order is always a correct sort order. This ordering sorts
	 * the two sets of face planes by their ascending normal-relative distance. The
	 * ordering between the two normals is irrelevant as they can't be seen through
	 * each other anyway.
	 * <p>
	...
	private SortType sortTypeHeuristic() {
		...
		if (!this.hasUnaligned) {
			...
			// special case B
			// if there are just two normals, they are exact opposites of each other and
			// they
		...
		var attemptLimitIndex = MathHelper.clamp_int(normalCount, 2, STATIC_TOPO_SORT_ATTEMPT_LIMITS.length - 1);
	...
	public TranslucentData getTranslucentData(TranslucentData oldData, BuiltSectionMeshParts translucentMesh,
			CombinedCameraPos cameraPos) {
