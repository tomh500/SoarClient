 ...
 * <p>
 * Implementation note: - Reusing the output of previous distance sorting job
 * doesn't make a difference or makes things slower in some cases. It's unclear
 * why exactly this happens, I suspect weird memory behavior or the reuse is not
 * actually that helpful to the sorting algorithm.
 */
public class DynamicTopoData extends DynamicData {
	...
	private DynamicTopoData(SectionPos sectionPos, int vertexCount, TQuad[] quads, GeometryPlanes geometryPlanes,
			Vector3dc initialCameraPos, Object2ReferenceOpenHashMap<Vector3fc, float[]> distancesByNormal) {
	...
	public Sorter getSorter() {
		return new DynamicTopoSorter(this.getQuadCount(), this, this.pendingTriggerIsDirect,
				this.consecutiveTopoSortFailures, this.GFNITrigger, this.directTrigger);
	}
	...
	public class DynamicTopoSorter extends DynamicSorter implements IntConsumer {
		...
		private DynamicTopoSorter(int quadCount, DynamicTopoData parent, boolean isDirectTrigger,
				int consecutiveTopoSortFailures, boolean GFNITrigger, boolean directTrigger) {
			super(quadCount);
			this.parent = parent;
		...
		void writeSort(CombinedCameraPos cameraPos, boolean initial) {
			...
			if (this.GFNITrigger && !this.isDirectTrigger) {
				...
				var result = TopoGraphSorting.topoGraphSort(this, DynamicTopoData.this.quads,
						DynamicTopoData.this.distancesByNormal, cameraPos.getRelativeCameraPos());
				this.intBuffer = null;
				...
				// ignored here.
				if (!initial && sortTime > (this.consecutiveTopoSortFailuresNew > 0 ? MAX_FAILING_TOPO_SORT_TIME_NS
	...
	public static DynamicTopoData fromMesh(int vertexCount, CombinedCameraPos cameraPos, TQuad[] quads,
			SectionPos sectionPos, GeometryPlanes geometryPlanes) {
		var distancesByNormal = geometryPlanes.prepareAndGetDistances();

		return new DynamicTopoData(sectionPos, vertexCount, quads, geometryPlanes, cameraPos.getAbsoluteCameraPos(),
				distancesByNormal);
	}
}
