...
import com.soarclient.libraries.soarium.render.chunk.occlusion.GraphDirectionSet;
...
/**
 * The render state object for a chunk section. This contains all the graphics
 * state for each render pass along with data about the render in the chunk
 * visibility graph.
 */
public class RenderSection {
	...
	/**
	 * Deletes all data attached to this render and drops any pending tasks. This
	 * should be used when the render falls out of view or otherwise needs to be
	 * destroyed. After the render has been destroyed, the object can no longer be
	 * used.
	 */
	public void delete() {
	...
	private boolean setRenderState(@NotNull BuiltSectionInfo info) {
		...
		// the section is marked as having received graph-relevant changes if it's build
		// state, flags, or connectedness has changed.
		// the entities and sprites don't need to be checked since whether they exist is
	...
	private boolean clearRenderState() {
		...
		// changes to data if it moves from built to not built don't matter, so only
		// build state changes matter
		return wasBuilt;
	}
	...
	/**
	 * @return The squared distance from the center of this chunk in the level to
	 *         the center of the block position given by {@param pos}
	 */
	...
	/**
	 * @return The squared distance from the center of this chunk to the given block
	 *         position
	 */
	...
	public String toString() {
		return String.format("RenderSection at chunk (%d, %d, %d) from (%d, %d, %d) to (%d, %d, %d)", this.chunkX,
				this.chunkY, this.chunkZ, this.getOriginX(), this.getOriginY(), this.getOriginZ(),
	...
	/**
	 * Returns a bitfield containing the {@link RenderSectionFlags} for this built
	 * section.
	 */
	public int getFlags() {
		return this.flags;
	...
	/**
	 * Returns the occlusion culling data which determines this chunk's
	 * connectedness on the visibility graph.
	 */
	...
	/**
	 * Returns the collection of animated sprites contained by this rendered chunk
	 * section.
	 */
	...
	/**
	 * Returns the collection of block entities contained by this rendered chunk,
	 * which are not part of its culling volume. These entities should always be
	 * rendered regardless of the render being visible in the frustum.
	 */
