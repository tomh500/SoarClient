 ...
 * 
 * TODO: - investigate why there's a similar number of STA and DYN sections.
 * This might be normal, the counters might be broken or the heuristic is
 * actually wrong.
 * 
...
public class SortTriggering {
	...
	public void applyTriggerChanges(DynamicTopoData data, DynamicTopoData.DynamicTopoSorter topoSorter, SectionPos pos,
			Vector3dc cameraPos) {
	...
	public void addDebugStrings(List<String> list) {
		var sortBehavior = Soarium.getConfig().performance.getSortBehavior();
		if (sortBehavior.getSortMode() == SortBehavior.SortMode.NONE) {
			list.add("TS OFF");
		} else {
			list.add("TS (%s) NL=%02d TrN=%02d TrS=G%03d/D%03d".formatted(sortBehavior.getShortName(),
					this.gfni.getUniqueNormalCount(), this.triggeredNormalCount, this.gfniTriggerCount,
					this.directTriggerCount));
