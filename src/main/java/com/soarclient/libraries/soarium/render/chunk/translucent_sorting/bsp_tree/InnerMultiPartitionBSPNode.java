...
class InnerMultiPartitionBSPNode extends InnerPartitionBSPNode {
	...
	InnerMultiPartitionBSPNode(NodeReuseData reuseData, int axis, float[] planeDistances, BSPNode[] partitions,
			int[][] onPlaneQuads) {
	...
	static BSPNode buildFromPartitions(BSPWorkspace workspace, IntArrayList indexes, int depth, BSPNode oldNode,
		...
		if (oldNode instanceof InnerMultiPartitionBSPNode multiNode && multiNode.axis == axis
				&& multiNode.partitions.length > 0) {
			...
			if (partition.quadsBefore() != null) {
				...
				if (oldPartitionNodes != null) {
					...
					if (oldChildIndex < oldPartitionNodes.length && (oldPartitionDistance == partitionDistance
							|| Float.isNaN(partitionDistance) && Float.isNaN(oldPartitionDistance))) {
		...
		return new InnerMultiPartitionBSPNode(prepareNodeReuse(workspace, indexes, depth), axis, planeDistances,
