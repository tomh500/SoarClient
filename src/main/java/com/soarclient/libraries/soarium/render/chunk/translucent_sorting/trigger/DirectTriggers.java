...
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.trigger.SortTriggering.SectionTriggers;
...
class DirectTriggers implements SectionTriggers<DynamicTopoData> {
	/**
	 * A tree map of the directly triggered sections, indexed by their minimum
	 * required camera movement. When the given camera movement is exceeded, they
	 * are tested for triggering the angle or distance condition.
	 * 
	...
	private static final double EARLY_TRIGGER_DISTANCE_SQUARED = Math.pow(TRIGGER_DISTANCE * EARLY_TRIGGER_FACTOR, 2);
	...
	private static class DirectTriggerData {
		...
		Vector3dc getSectionCenter() {
			if (this.sectionCenter == null) {
				this.sectionCenter = new Vector3d(sectionPos.minBlockX() + 8, sectionPos.minBlockY() + 8,
		...
		double centerRelativeAngleCos(Vector3dc a, Vector3dc b) {
			...
			return angleCos(sectionCenter.x() - a.x(), sectionCenter.y() - a.y(), sectionCenter.z() - a.z(),
					sectionCenter.x() - b.x(), sectionCenter.y() - b.y(), sectionCenter.z() - b.z());
		}
