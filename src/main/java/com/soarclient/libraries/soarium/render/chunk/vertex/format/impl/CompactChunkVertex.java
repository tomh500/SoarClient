...
import com.soarclient.libraries.soarium.render.chunk.shader.ChunkShaderBindingPoints;
...
import com.soarclient.libraries.soarium.render.chunk.vertex.format.ChunkVertexEncoder.Vertex;
...
public class CompactChunkVertex implements ChunkVertexType {
	...
	public static final GlVertexFormat VERTEX_FORMAT = GlVertexFormat.builder(STRIDE)
			...
			.addElement(DefaultChunkMeshAttributes.LIGHT_MATERIAL_INDEX,
					ChunkShaderBindingPoints.ATTRIBUTE_LIGHT_MATERIAL_INDEX, 16)
			.build();
	...
	private static int encodeTexture(float center, float x) {
		// Shrink the texture coordinates (towards the center of the mapped texture
		// region) by the minimum
		// addressable unit (after quantization.) Then, encode the sign of the bias that
		// was used, and apply
		// the inverse transformation on the GPU with a small epsilon.
		//
		// This makes it possible to use much smaller epsilons for avoiding texture
		// bleed, since the epsilon is no
