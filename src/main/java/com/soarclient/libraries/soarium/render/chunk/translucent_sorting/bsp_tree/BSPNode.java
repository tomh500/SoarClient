 ...
 * 
 * Implementation note: - Doing a convex box test doesn't seem to bring a
 * performance boost, even if it does trigger sometimes with man-made
 * structures. The multi partition node probably does most of the work already.
	...
	private static boolean doubleLeafPossible(TQuad quadA, TQuad quadB) {
		...
		if (!facingA.isAligned() || !facingB.isAligned()) {
			...
			if (NormI8.isOpposite(packedNormalA, packedNormalB)
					...
					|| packedNormalA == packedNormalB
							&& quadA.getAccurateDotProduct() == quadB.getAccurateDotProduct()) {
				return true;
			}
		}
