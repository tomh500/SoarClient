...
import com.soarclient.libraries.soarium.render.chunk.compile.BuilderTaskOutput;
...
import com.soarclient.libraries.soarium.render.chunk.compile.executor.ChunkJobCollector;
import com.soarclient.libraries.soarium.render.chunk.compile.executor.ChunkJobResult;
import com.soarclient.libraries.soarium.render.chunk.compile.tasks.ChunkBuilderMeshingTask;
import com.soarclient.libraries.soarium.render.chunk.compile.tasks.ChunkBuilderSortingTask;
import com.soarclient.libraries.soarium.render.chunk.compile.tasks.ChunkBuilderTask;
...
import com.soarclient.libraries.soarium.render.chunk.lists.VisibleChunkCollector;
import com.soarclient.libraries.soarium.render.chunk.occlusion.GraphDirection;
...
import com.soarclient.libraries.soarium.render.chunk.region.RenderRegionManager;
import com.soarclient.libraries.soarium.render.chunk.terrain.TerrainRenderPass;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.SortBehavior.DeferMode;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.SortBehavior.PriorityMode;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.data.DynamicTopoData;
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.data.NoData;
...
import com.soarclient.libraries.soarium.render.chunk.translucent_sorting.trigger.SortTriggering;
import com.soarclient.libraries.soarium.render.chunk.vertex.format.ChunkMeshFormats;
import com.soarclient.libraries.soarium.render.texture.SpriteUtil;
...
import com.soarclient.libraries.soarium.util.CameraUtils;
...
public class RenderSectionManager {
	...
	public RenderSectionManager(WorldClient level, int renderDistance, CommandList commandList) {
	...
	private float getSearchDistance() {
		...
		if (Soarium.getConfig().performance.useFogOcclusion) {
	...
	private boolean shouldUseOcclusionCulling(boolean spectator) {
		...
		if (spectator && this.level.getBlockState(origin).getBlock().isFullBlock()) {
			...
			useOcclusionCulling = Soarium.getConfig().performance.smartCull;
		}
	...
	public void uploadChunks() {
		...
		// only mark as needing a graph update if the uploads could have changed the
		// graph
		// (sort results never change the graph)
		// generally there's no sort results without a camera movement, which would also
		// trigger
		...
		for (var result : filtered) {
			...
			if (result instanceof ChunkBuildOutput chunkBuildOutput) {
				...
				if (chunkBuildOutput.translucentData != null) {
					...
					// a rebuild always generates new translucent data which means
					// applyTriggerChanges isn't necessary
					result.render.setTranslucentData(chunkBuildOutput.translucentData);
				}
			} else if (result instanceof ChunkSortOutput sortOutput && sortOutput.getTopoSorter() != null
					&& result.render.getTranslucentData() instanceof DynamicTopoData data) {
				this.sortTriggering.applyTriggerChanges(data, sortOutput.getTopoSorter(), result.render.getPosition(),
						this.cameraPosition);
			}
	...
	public void updateChunks(boolean updateImmediately) {
		...
		if (updateImmediately) {
			// for a perfect frame where everything is finished use the last frame's
			// blocking collector
			...
			var deferredCollector = new ChunkJobCollector(this.builder.getHighEffortSchedulingBudget(),
					this.builder.getLowEffortSchedulingBudget(), this.buildResults::add);

			// if zero frame delay is allowed, submit important sorts with the current frame
			// blocking collector.
			// otherwise submit with the collector that the next frame is blocking on.
			if (Soarium.getConfig().performance.getSortBehavior().getDeferMode() == DeferMode.ZERO_FRAMES) {
			...
			// wait on this frame's blocking collector which contains the important tasks
			// from this frame
	...
	private void submitSectionTasks(ChunkJobCollector importantCollector, ChunkJobCollector semiImportantCollector,
		...
		while (!queue.isEmpty() && collector.hasBudgetFor(type.getTaskEffort(), ignoreEffortCategory)) {
			...
			if (type == ChunkUpdateType.SORT || type == ChunkUpdateType.IMPORTANT_SORT) {
				...
				if (task == null) {
					...
					// index data.
					var result = ChunkJobResult.successfully(
							new ChunkBuildOutput(section, frame, NoData.forEmptySection(section.getPosition()),
	...
	public void scheduleSort(long sectionPos, boolean isDirectTrigger) {
		...
		if (section != null) {
			...
			var priorityMode = Soarium.getConfig().performance.getSortBehavior().getPriorityMode();
			if (priorityMode == PriorityMode.ALL || priorityMode == PriorityMode.NEARBY
					&& this.shouldPrioritizeTask(section, NEARBY_SORT_DISTANCE)) {
	...
	public void scheduleRebuild(int x, int y, int z, boolean important) {
		...
		if (section != null && section.isBuilt()) {
			...
			if (allowImportantRebuilds()
					&& (important || this.shouldPrioritizeTask(section, NEARBY_REBUILD_DISTANCE))) {
	...
	private static boolean allowImportantRebuilds() {
		return !Soarium.getConfig().performance.alwaysDeferChunkUpdates;
	}
	...
	private void connectNeighborNodes(RenderSection render) {
		for (int direction = 0; direction < GraphDirection.COUNT; direction++) {
			RenderSection adj = this.getRenderSection(render.getChunkX() + GraphDirection.x(direction),
					render.getChunkY() + GraphDirection.y(direction), render.getChunkZ() + GraphDirection.z(direction));
	...
	public Collection<String> getDebugStrings() {
		...
		list.add(String.format("Geometry Pool: %d/%d MiB (%d buffers)", MathUtil.toMib(deviceUsed),
				MathUtil.toMib(deviceAllocated), count));
		list.add(String.format("Transfer Queue: %s", this.regions.getStagingBuffer().toString()));

		list.add(String.format("Chunk Builder: Permits=%02d (E %03d) | Busy=%02d | Total=%02d",
				this.builder.getScheduledJobCount(), this.builder.getScheduledEffort(),
				this.builder.getBusyThreadCount(), this.builder.getTotalThreadCount()));
		...
		list.add(String.format("Chunk Queues: U=%02d (P0=%03d | P1=%03d | P2=%03d)", this.buildResults.size(),
				this.taskLists.get(ChunkUpdateType.IMPORTANT_REBUILD).size()
						+ this.taskLists.get(ChunkUpdateType.IMPORTANT_SORT).size(),
