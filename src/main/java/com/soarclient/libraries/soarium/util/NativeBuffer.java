...
public class NativeBuffer {
	...
	private static final Reference2ReferenceMap<Reference<NativeBuffer>, BufferReference> ACTIVE_BUFFERS = Reference2ReferenceMaps
			.synchronize(new Reference2ReferenceOpenHashMap<>());
		...
		while ((ref = RECLAIM_QUEUE.poll()) != null) {
			...
			if (buf.allocationSite != null) {
				LOGGER.warn("Reclaimed {} bytes at address {} that were leaked from allocation site:\n{}", buf.length,
						buf.address, Arrays.stream(buf.allocationSite).map(StackTraceElement::toString)
								.collect(Collectors.joining("\n")));
			} else {
				LOGGER.warn(
						"Reclaimed {} bytes at address {} that were leaked from an unknown location (logging is disabled)",
	...
	private static StackTraceElement[] getStackTrace() {
		return Soarium.getConfig().advanced.enableMemoryTracing ? Thread.currentThread().getStackTrace() : null;
	}
	...
	private static BufferReference allocate(int bytes) {
		...
		while (++attempts <= MAX_ALLOCATION_ATTEMPTS) {
			...
			LOGGER.error("EMERGENCY: ... Attempting to force a garbage collection cycle (attempt {}/{})", attempts,
