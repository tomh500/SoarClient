...
import com.soarclient.libraries.soarium.compat.minecraft.math.QuartPos;
import com.soarclient.libraries.soarium.compat.minecraft.random.LinearCongruentialGenerator;
...
public class LevelBiomeSlice {
	...
	public BiomeGenBase getBiome(int blockX, int blockY, int blockZ) {
		...
		int centerIndex = dataArrayIndex(QuartPos.fromBlock(relBlockX - 2), QuartPos.fromBlock(relBlockY - 2),
	...
	private BiomeGenBase getBiomeUsingVoronoi(int blockX, int blockY, int blockZ) {
		...
		// The distance is calculated between center positions, which are offset by the
		// bias parameter
