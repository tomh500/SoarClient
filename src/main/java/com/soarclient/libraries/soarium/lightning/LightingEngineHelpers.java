package com.soarclient.libraries.soarium.lightning;
...
public class LightingEngineHelpers {
    ...
    public static Chunk getLoadedChunk(IChunkProvider chunkProvider, int x, int z) {
        if (chunkProvider instanceof ChunkProviderServer) {
        	Long2ObjectMap<Chunk> chunkStorage = ((ChunkProviderServer)chunkProvider).getId2ChunkMap();
            return chunkStorage.get(ChunkCoordIntPair.chunkXZ2Int(x, z));
        }
        if (chunkProvider instanceof ChunkProviderClient) {
        	Long2ObjectMap<Chunk> chunkStorage = ((ChunkProviderClient)chunkProvider).getLoadedChunks();
            return chunkStorage.get(ChunkCoordIntPair.chunkXZ2Int(x, z));
        }
