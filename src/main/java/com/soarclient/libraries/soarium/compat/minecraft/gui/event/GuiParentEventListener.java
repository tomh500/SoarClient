package com.soarclient.libraries.soarium.compat.minecraft.gui.event;
...
public interface GuiParentEventListener extends GuiEventListener {
	...
	default boolean mouseScrolled(double mouseX, double mouseY, double verticalAmount, double amount) {
		return this.hoveredElement(mouseX, mouseY)
				.filter((element) -> element.mouseScrolled(mouseX, mouseY, verticalAmount, amount)).isPresent();
	}
	...
	default boolean changeFocus(boolean lookForwards) {
		...
		if (element == null || !element.changeFocus(lookForwards)) {
			...
			final Supplier<? extends GuiEventListener> supplier = lookForwards ? listIterator::next
