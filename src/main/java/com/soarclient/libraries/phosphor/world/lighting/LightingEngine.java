package com.soarclient.libraries.phosphor.world.lighting;

import java.util.concurrent.locks.ReentrantLock;

import com.soarclient.libraries.phosphor.api.IChunkLighting;
import com.soarclient.libraries.phosphor.api.ILightingEngine;
import com.soarclient.libraries.phosphor.world.collections.PooledLongQueue;
...
public class LightingEngine implements ILightingEngine {
	...
	static {
		for (int i = 0; i < 6; ++i) {
			...
			neighborShifts[i] = ((long) offset.getY() << sY) | ((long) offset.getX() << sX)
					| ((long) offset.getZ() << sZ);
		}
	}
	 ...
	 * processed later by
	 * {@link ILightingEngine#processLightUpdatesForType(EnumSkyBlock)}
	 */
	@Override
	...
	/**
	 * Schedules a light update for the specified light type and position to be
	 * processed later by {@link ILightingEngine#processLightUpdates()}
	 */
	...
	/**
	 * Calls {@link ILightingEngine#processLightUpdatesForType(EnumSkyBlock)} for
	 * both light types
	 */
	@Override
			...
			while (this.nextItem()) {
				...
				if (this.calculateNewLightFromCursor(luminosity, opacity, lightType) < curLight) {
					...
					for (NeighborInfo info : this.neighborInfos) {
						...
						if (curLight - this.getPosOpacity(nPos,
								LightingEngineHelpers.posToState(nPos, info.section)) >= nLight) // schedule neighbor
																									// for darkening if
																									// we possibly light
																									// it
						{
				...
				} else // we didn't become darker, so we need to re-set our initial light value (was
						// set to 0) and notify neighbors
				{
					this.enqueueBrighteningFromCursor(curLight, lightType); // do not spread to neighbors immediately to
	...
	private void spreadLightFromCursor(final int curLight, final EnumSkyBlock lightType) {
		...
		for (NeighborInfo info : this.neighborInfos) {
			...
			final int newLight = curLight
					- this.getPosOpacity(info.pos, LightingEngineHelpers.posToState(info.pos, info.section));
	...
	private int getCursorLuminosity(final IBlockState state, final EnumSkyBlock lightType) {
		...
		return MathHelper.clamp_int(LightingEngineHelpers.getLightValueForState(state, this.world, this.curPos), 0,
				MAX_LIGHT);
	}
