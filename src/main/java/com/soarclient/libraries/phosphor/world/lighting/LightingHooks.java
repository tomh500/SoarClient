...
public class LightingHooks {
		...
		if (emptySections != 0) {
			for (final EnumFacing dir : EnumFacing.HORIZONTALS) {
				...
				final boolean neighborColumnExists = (((x + xOffset) | (z + zOffset)) & 16) == 0
						...
						// both 15+1 and 0-1)
						|| LightingEngineHelpers.getLoadedChunk(world.getChunkProvider(), chunk.xPosition + xOffset,
	...
	public static void scheduleRelightChecksForChunkBoundaries(final World world, final Chunk chunk) {
		for (final EnumFacing dir : EnumFacing.HORIZONTALS) {
			...
			final Chunk nChunk = LightingEngineHelpers.getLoadedChunk(world.getChunkProvider(),
					chunk.xPosition + xOffset, chunk.zPosition + zOffset);
	...
	private static void scheduleRelightChecksForBoundary(final World world, final Chunk chunk, Chunk nChunk,
		...
		if (nChunk == null) {
			nChunk = LightingEngineHelpers.getLoadedChunk(world.getChunkProvider(), chunk.xPosition + xOffset,
		...
		if (sChunk == null) {
			sChunk = LightingEngineHelpers.getLoadedChunk(world.getChunkProvider(),
					chunk.xPosition + (zOffset != 0 ? axisDir.getOffset() : 0),
					chunk.zPosition + (xOffset != 0 ? axisDir.getOffset() : 0));
	...
	public static void checkChunkLighting(final Chunk chunk, final World world) {
		...
		for (int x = -1; x <= 1; ++x) {
			for (int z = -1; z <= 1; ++z) {
				if (x != 0 || z != 0) {
					Chunk nChunk = LightingEngineHelpers.getLoadedChunk(world.getChunkProvider(), chunk.xPosition + x,
							chunk.zPosition + z);
