package com.soarclient.libraries.phosphor.world;

import com.soarclient.libraries.phosphor.world.lighting.LightingEngineHelpers;
	...
	public WorldChunkSlice(World world, int x, int z) {
		...
		for (int xDiff = -radius; xDiff <= radius; xDiff++) {
			for (int zDiff = -radius; zDiff <= radius; zDiff++) {
				this.chunks[((xDiff + radius) * DIAMETER) + (zDiff + radius)] = LightingEngineHelpers
						.getLoadedChunk(world.getChunkProvider(), x + xDiff, z + zDiff);
			}
		}
