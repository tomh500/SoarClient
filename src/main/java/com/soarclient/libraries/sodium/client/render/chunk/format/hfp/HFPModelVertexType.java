...
public class HFPModelVertexType implements ChunkVertexType {
	public static final GlVertexFormat<ChunkMeshAttribute> VERTEX_FORMAT = GlVertexFormat
			.builder(ChunkMeshAttribute.class, 20)
			...
			.addElement(ChunkMeshAttribute.LIGHT, 16, GlVertexAttributeFormat.UNSIGNED_SHORT, 2, true).build();
	...
	public ModelVertexSink createBufferWriter(VertexBufferView buffer, boolean direct) {
		return (ModelVertexSink) (direct ? new HFPModelVertexBufferWriterUnsafe(buffer)
				: new HFPModelVertexBufferWriterNio(buffer));
	}
