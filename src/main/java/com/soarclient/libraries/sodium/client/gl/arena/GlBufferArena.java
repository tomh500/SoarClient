package com.soarclient.libraries.sodium.client.gl.arena;
...
public class GlBufferArena {
	...
	public void free(GlBufferSegment segment) {
		if (this.freeRegions.contains(segment)) {
			...
			GlBufferSegment prev = null;
			GlBufferSegment next = null;
	...
	private GlBufferSegment allocReuse(int len) {
		...
		for (GlBufferSegment segment : this.freeRegions) {
			if (segment.getLength() >= len && (bestSegment == null || bestSegment.getLength() > segment.getLength())) {
				bestSegment = segment;
			}
		}
