...
import com.soarclient.libraries.sodium.client.model.light.LightMode;
...
import com.soarclient.libraries.sodium.client.model.quad.ModelQuad;
import com.soarclient.libraries.sodium.client.model.quad.ModelQuadView;
import com.soarclient.libraries.sodium.client.model.quad.ModelQuadViewMutable;
...
import com.soarclient.libraries.sodium.client.render.chunk.compile.buffers.ChunkModelBuffers;
import com.soarclient.libraries.sodium.client.render.chunk.format.ModelVertexSink;
import com.soarclient.libraries.sodium.client.util.MathUtil;
...
import com.soarclient.libraries.sodium.fluid.EmbeddiumFluidSpriteCache;
...
public class FluidRenderer {
	...
	private boolean isFluidOccluded(IBlockAccess world, int x, int y, int z, EnumFacing dir, BlockLiquid fluid) {
		...
		BlockPos adjPos = this.scratchPos.set(x + dir.getFrontOffsetX(), y + dir.getFrontOffsetY(),
				z + dir.getFrontOffsetZ());
		BlockLiquid adjFluid = WorldUtil.getFluid(world.getBlockState(adjPos));
		boolean temp = fluid == adjFluid;
		return !blockState.getBlock().getMaterial().isOpaque() ? temp
				: temp || blockState.getBlock().shouldSideBeRendered(world, pos, dir);
	}

	private boolean isSideExposed(IBlockAccess world, int x, int y, int z, EnumFacing dir) {
		BlockPos pos = this.scratchPos.set(x + dir.getFrontOffsetX(), y + dir.getFrontOffsetY(),
				z + dir.getFrontOffsetZ());
		IBlockState blockState = world.getBlockState(pos);
