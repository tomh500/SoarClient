...
public class ScrollBarComponent extends AbstractWidget {
	...
	public ScrollBarComponent(Dim2i trackArea, ScrollBarComponent.Mode mode, int frameLength, int viewPortLength,
			Consumer<Integer> onSetOffset) {
		this.dim = trackArea;
		this.mode = mode;
	...
	public ScrollBarComponent(Dim2i scrollBarArea, ScrollBarComponent.Mode mode, int frameLength, int viewPortLength,
			Consumer<Integer> onSetOffset, Dim2i extendedTrackArea) {
	...
	public void render(int mouseX, int mouseY, float delta) {
		...
		this.drawRect((double) this.scrollThumb.getOriginX(), (double) this.scrollThumb.getOriginY(),
				(double) this.scrollThumb.getLimitX(), (double) this.scrollThumb.getLimitY(), -5592406);
	}
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		if (this.dim.containsCursor(mouseX, mouseY)) {
			if (this.scrollThumb.containsCursor(mouseX, mouseY)) {
				...
				int value;
				if (this.mode == ScrollBarComponent.Mode.VERTICAL) {
					value = (int) ((mouseY - (double) this.dim.getOriginY()
							- (double) (this.scrollThumb.getHeight() / 2))
							/ (double) (this.dim.getHeight() - this.scrollThumb.getHeight())
							* (double) this.maxScrollBarOffset);
				} else {
					value = (int) ((mouseX - (double) this.dim.getOriginX()
							- (double) (this.scrollThumb.getWidth() / 2))
	...
	public boolean mouseDragged(double mouseX, double mouseY, int button) {
		if (this.isDragging) {
			int value;
			if (this.mode == ScrollBarComponent.Mode.VERTICAL) {
				value = (int) ((mouseY - (double) this.scrollThumbClickOffset - (double) this.dim.getOriginY()
						- (double) (this.scrollThumb.getHeight() / 2))
						/ (double) (this.dim.getHeight() - this.scrollThumb.getHeight())
						* (double) this.maxScrollBarOffset);
			} else {
				value = (int) ((mouseX - (double) this.scrollThumbClickOffset - (double) this.dim.getOriginX()
						- (double) (this.scrollThumb.getWidth() / 2))
						/ (double) (this.dim.getWidth() - this.scrollThumb.getWidth())
						* (double) this.maxScrollBarOffset);
			}
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
		if ((this.dim.containsCursor(mouseX, mouseY)
				|| this.extendedScrollArea != null && this.extendedScrollArea.containsCursor(mouseX, mouseY))
				&& this.offset <= this.maxScrollBarOffset && this.offset >= 0) {
