...
public class WavWriter {
	...
	public void writeHeader() throws IOException {
		...
		if (totalSamples == 0) {
			if (!(os instanceof RandomAccessFile))
				throw new IOException("Cannot seek in output stream");
			needsFixup = true;
		}
		// if (dataSize >= 0xFFFFFFDC) throw new IOException("ERROR: stream is too big
		// to fit in a single file chunk (Datasize="+dataSize+")");

		os.write("RIFF".getBytes());
		if (needsFixup)
			riffOffset = ((RandomAccessFile) os).getFilePointer();
		...
		if (needsFixup)
			dataOffset = ((RandomAccessFile) os).getFilePointer();
