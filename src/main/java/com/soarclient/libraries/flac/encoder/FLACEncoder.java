 ...
 * StreamConfiguration object is used as the actual block size. <br>
 * <br>
 * <br>
 * An encoding process is simple, and should follow these steps:<br>
 * <BLOCKQUOTE> 1) Set StreamConfiguration to appropriate values. After a stream
 * is opened, this must not be altered until the stream is closed.<br>
 ...
 * executing(as in threaded mode). See related method documentation for info on
 * concurrent use) (note: steps 4,5, and 6 may be done repeatedly, in any order,
 * with the exception that step 4 must not be called while a concurrent step 6
 * is executing(as in threaded mode). See related method documentation for info
...
public class FLACEncoder {
	...
	/**
	 * Maximum Threads to use for encoding frames(more threads than this will exist,
	 * these threads are reserved for encoding of frames only).
	 */
	...
	/*
	 * Set true if frames are actively being encoded(can't change settings while
	 * this is true). Use streamLock for changing.
	 */
	volatile Boolean isEncoding = false;

	/**
	 * we set this to true while a flac stream has been opened and not officially
	 * closed. Must use a streamLock to set and get this. Some actions must not be
	 * taken while a stream is opened
	 */
	...
	/*
	 * set when an IOException has occured that invalidates results in a child
	 * encoding thread. IOException temporarily stored by childException when this
	 * is true.
	 */
	boolean error = false;
	/*
	 * Throw this if exists, when we can, to notify main thread an exception occured
	 * in a child thread.
	 */
	 ...
	 * StreamConfiguration should be reset to match the audio used and an output
	 * stream set, but the default EncodingConfiguration should be ok for most
	 * purposes. When using threaded encoding, the default number of threads used is
	 * equal to FLACEncoder.MAX_THREADED_FRAMES.
	 */
	public FLACEncoder() {
		...
		try {
			...
			throw new IllegalStateException(
					"Error! FLACEncoder cannot function" + "without a valid MD5 implementation.", e);
		}
	}

	/**
	 * Tell encoder how many threads to use for encoding. More threads than this
	 * will exist, but only the given amount should be in a running state at any
	 * moment(the other threads are simply manager threads, waiting for
	 * encoding-threads to end). A special case is setting "count" to zero; this
	 * will tell the encoder not to use internal threads at all, and all encoding
	 * will be done with the main thread. Otherwise, any encode methods will return
	 * while the encode actually takes place in a separate thread.
	 *
	 * @param count Number of encoding threads to use. Count &gt; 0 means use that
	 *              many independent encoding threads, count == 0 means encode in
	 *              main thread, count &lt; 0 is ignored.
	 *
	 * @return boolean value represents whether requested count was applied or not.
	 *         This may be false if a FLAC stream is currently opened.
	 */
	...
	/**
	 * Creates and configures a new BlockThreadManager if needed(or sets to null if
	 * threads turned off.) Method must only be called if flacStreamIsOpen equals
	 * false, and this must not change while executing this method.
	 * 
	 * @param sc
	 */
	 ...
	 * configuration is not stored by this object, but instead copied. This is to
	 * prevent the alteration of the config during an encode process. This must not
	 * be called while an encodeSamples(...) is active, or while encoding threads
	 * are active. If using threaded mode, use a blocking-count of zero in
	 * t_encodeSamples(...)to ensure the underlying encoding threads have finished
	 ...
	 * configuration is not stored by this object, but instead copied. This is to
	 * prevent the alteration of the config during an encode process. This method
	 * must not be called in the middle of a stream, stream contents may become
	 * invalid. Calling this method clears any data stored by this encoder. A call
	 * to setStreamConfiguration() should be followed next by setting the output
	 * stream if not yet done, and then calling openFLACStream();
	 *
	...
	/**
	 * Reset the values to their initial state, in preparation of starting a new
	 * stream. Does *not* clear any stored, unwritten data. To flush stored samples,
	 * call clear().
	 */
	private void reset() {
		md5.getMD().reset();
	 ...
	 * unless you desire to keep unencoded samples. This does NOT reset or close the
	 * active stream.
	 */
	...
	/**
	 * Close the current FLAC stream. Updates the stream header information. If
	 * called on a closed stream, operation is undefined. Do not do this.
	 */
	 ...
	 * StreamConfiguration and the output stream, both of which must not change
	 * until encoding is finished and the stream is closed. If this FLACEncoder
	 * object has already been used to encode a stream, unencoded samples may still
	 * be stored. Use clear() to dump them prior to calling this method(if clear()
	 * not called, and samples are instead retained, the StreamConfiguration must
	 * NOT have changed from the prior stream.
	 *
	 ...
	 * breaks the samples into blocks, which will then be made available to encode.
	 *
	 * @param samples Array holding the samples to encode. For all multi-channel
	 *                audio, the samples must be interleaved in this array. For
	 *                example, with stereo: sample 0 will belong to the first
	 *                channel, 1 the second, 2 the first, 3 the second, etc. Samples
	 *                are interpreted according to the current configuration(for
	 *                things such as channel and bits-per-sample).
	 *
	 * @param count   Number of interchannel samples to add. For example, with
	 *                stero: if this is 4000, then "samples" must contain 4000 left
	 *                samples and 4000 right samples, interleaved in the array.
	 */
	...
	private void writeFinishedBlock(BlockEncodeRequest ber) throws IOException {
		...
		md5.addSamplesToMD5(ber.samples, ber.encodedSamples, ber.skip + 1, streamConfig.getBitsPerSample());
	...
	/**
	 * Notify the encoder that a BlockEncodeRequest has finished, and is now ready
	 * to be written to file. The encoder expects that these requests come back in
	 * the same order the encoder sent them out. This is intended to be used in
	 * threading mode only at the moment(sending them to a BlockThreadManager
	 * object)
	 *
	 ...
	 * thread. This method should be called regularly in any public method to let
	 * the calling thread know a problem occured.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Attempt to Encode a certain number of samples(threaded version). Encodes as
	 * close to count as possible. Uses multiple threads to speed up encoding. If
	 * getThreadCount() &le; 0, simply calls the non-threaded version,
	 * encodeSamples(...), and blocks until it returns.
	 *
	 * @param inCount       number of samples to attempt to encode. Actual number
	 *                      encoded may be greater or less if count does not end on
	 *                      a block boundary. If "end" is false, we may set this
	 *                      value to something absurdly high, such as
	 *                      Integer.MAX_VALUE to ensure all available, full blocks
	 *                      are encoded.
	 *
	 * @param end           true to finalize stream after encode, false otherwise.
	 *                      If set to true, and return value is greater than or
	 *                      equal to given count, no more encoding must be attempted
	 *                      until a new stream is began.
	 *
	 * @param blockingCount value is used for flow-control into this encoder. This
	 *                      method will block until fewer than the given number of
	 *                      blocks remain queued for encoding.
	 *
	 * @return number of samples encoded. This may be greater or less than requested
	 *         count if count does not end on a block boundary. This is NOT an error
	 *         condition. If end was set "true", and returned count is less than
	 *         requested count, then end was NOT done, if you still wish to end
	 *         stream, call this again with end true and a count of of &le;
	 *         samplesAvailableToEncode()
	 *
	 * @throws IOException if there was an error writing the results to output
	 *                     stream.
	 */
	public int t_encodeSamples(final int inCount, final boolean end, int blockingCount) throws IOException {
		int count = inCount;
		if (MAX_THREADED_FRAMES <= 0)
	...
	/**
	 * Attempt to Encode a certain number of samples. Encodes as close to count as
	 * possible.
	 *
	 * @param count number of samples to attempt to encode. Actual number encoded
	 *              may be greater or less if count does not end on a block
	 *              boundary.
	 *
	 * @param end   true to finalize stream after encode, false otherwise. If set to
	 *              true, and return value is greater than or equal to given count,
	 *              no more encoding must be attempted until a new stream is began.
	 *
	 * @return number of samples encoded. This may be greater or less than requested
	 *         count if count does not end on a block boundary. This is NOT an error
	 *         condition. If end was set "true", and returned count is less than
	...
	private int encodeRequest(BlockEncodeRequest ber, int channels) throws IOException {
		...
		int encoded = frame.encodeSamples(block, encodedSamples, 0, channels - 1, result, ber.frameNumber);
	...
	/**
	 * Get number of samples which are ready to encode. More samples may exist in
	 * the encoder as a partial block. Use samplesAvailableToEncode() if you wish to
	 ...
	 * which are in a partial block(and so would only be written if "end" was set
	 * true in encodeSamples(int count,boolean end);
	 * 
	...
	/**
	 * Set the output stream to use. This must not be called while an encode process
	 * is active, or a flac stream is already opened.
	 * 
