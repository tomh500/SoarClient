...
public class EncodedElement {
	...
	/**
	 * Previous element in list. At current times, this should not be dependend on
	 * to be set
	 */
	...
	/**
	 * Data stored by this element. Member usableBits should be used to track the
	 * last valid index at a bit level.
	 */
	byte[] data = null;

	/**
	 * Use to track the last valid index of the data array at a bit level. For
	 * example, a value of "10" would mean the first byte and two low-order bits of
	 * the second byte are used. usableBits must always be equal or greater than
	 * offset.
	 */
	 ...
	 * purposes of speed, it is not always best to pack the data starting at bit
	 * zero of first byte. offset must always be less than or equal to usableBits.
	 */
	protected int offset;

	/**
	 * Constructor, creates an empty element with offset of zero and array size of
	 * 100. This array can be replaced with a call to setData(...).
	 */
	...
	/**
	 * Completely clear this element and use the given size and offset for the new
	 * data array.
	 *
	 * @param size Size of data array to use(in bytes)
	 * @param off  Offset to use for this element. Usablebits will also be set to
	 ...
	 *
	 *         Precondition: none Post-condition: getPrevious() will now return the
	 *         given object. Any existing “previous” was lost.
	 */
	 ...
	 * @return void Pre-condition: none Post-condition: getNext() will now return
	 *         the given object. Any existing “next” was lost.
	 */
	 ...
	 *
	 *             Pre-condition: None Post-condition: 'data' is now stored by this
	 *             object. Any previous data stored was lost.
	 */
	 ...
	 *
	 * @return the data stored in this byte[] is likely not all usable - method
	 *         getUsableBits() should be used to determine such.
	 */
	byte[] getData() {
		return data;
	}
	 ...
	 * provide minor speed improvement. Loops through all elements' "next" pointers,
	 * till the last is found.
	 * 
	...
	/**
	 * Return the last element of the list given. Loops through all elements' "next"
	...
	/**
	 * Add a number of bits from a long to the end of this list's data. Will add a
	 * new element if necessary. The bits stored are taken from the lower- order of
	 * input.
	 *
	...
	/**
	 * Add a number of bits from an int to the end of this list's data. Will add a
	 * new element if necessary. The bits stored are taken from the lower- order of
	 * input.
	 *
	...
	public EncodedElement addInt(int input, int bitCount) {
		if (next != null) {
			...
			next = new EncodedElement(size, tOff);
			System.err.println("creating next node of size:bitCount " + size + ":" + bitCount + ":" + usableBits + ":"
					+ data.length);
			...
			/*
			 * if(startPos/8+8 > dest.length) EncodedElement.addInt(input, bitCount,
			 * startPos, dest); else EncodedElement.addInt_new(input, bitCount, startPos,
			 * dest);
			 */
			usableBits += bitCount;
			return this;
	...
	/**
	 * Append an equal number of bits from each int in an array within given limits
	 ...
	 * @return EncodedElement containing end of packed data. Data may flow between
	 *         multiple EncodedElement's, if an existing element was not large
	...
	/**
	 * Pack a number of bits from each int of an array(within given limits)to the
	 * end of this list.
	 *
	 * @param inputA      Array containing input values.
	 * @param inputBits   Array containing number of bits to use for each index
	 *                    packed. This array should be equal in size to the inputA
	 *                    array.
	...
	public EncodedElement packIntByBits(int[] inputA, int[] inputBits, int inputOffset, int countA) {
		...
		// pack them and update usable bits.
		if (writeCount > 0) {
			EncodedElement.packIntByBits(inputA, inputBits, inputOffset, writeCount, usableBits, data);
	 ...
	 * { if(DEBUG_LEV > 30) System.err.println("EncodedElement::addInt : Begin");
	 * int currentByte = startPos/8; int currentOffset = startPos%8; int
	 * bitRoom;//how many bits can be placed in current byte int upMask;//to clear
	 * upper bits(lower bits auto-cleared by L-shift int downShift;//bits to shift
	 * down, isolating top bits of input int upShift;//bits to shift up, packing
	 * byte from top. while(count > 0) { //find how many bits can be placed in
	 * current byte bitRoom = 8-currentOffset; //get those bits //i.e, take upper
	 * 'bitsNeeded' of input, put to lower part of byte. downShift = count-bitRoom;
	 * upMask = 255 >>> currentOffset; upShift = 0; if(downShift < 0) { //upMask =
	 * 255 >>> bitRoom-count; upShift = bitRoom - count; upMask = 255 >>>
	 * (currentOffset+upShift); downShift = 0; } if(DEBUG_LEV > 30) {
	 * System.err.println("count:offset:bitRoom:downShift:upShift:" +
	 * count+":"+currentOffset+":"+bitRoom+":"+downShift+":"+upShift); } int
	 * currentBits = (input >>> downShift) & (upMask); //shift bits back up to match
	 * offset currentBits = currentBits << upShift; upMask = (byte)upMask <<
	 * upShift;
	 * 
	 * dest[currentByte] = (byte)(dest[currentByte] & (~upMask)); //merge bytes~
	 * dest[currentByte] = (byte)(dest[currentByte] | currentBits);
	 * //System.out.println("new currentByte: " + dest[currentByte]); count -=
	 * bitRoom; currentOffset = 0; currentByte++; } if(DEBUG_LEV > 30)
	...
	private static void addLong(long input, int count, int startPos, byte[] dest) {
		...
		while (count > 0) {
			...
			if (DEBUG_LEV > 30) {
				System.err.println("count:offset:bitRoom:downShift:upShift:" + count + ":" + currentOffset + ":"
	...
	/**
	 * public static void packIntOLD_WORKING(int[] inputArray, int startBitSize, int
	 * startPos, int start, int skip, int count, byte[] dest) { if(DEBUG_LEV > 0)
	 * System.err.println("EncodedElement::packInt : Begin"); if(DEBUG_LEV > 10)
	 * System.err.println("start:skip:count : " +start+":"+skip+":"+count); for(int
	 * i = 0; i < count; i++) { addInt(inputArray[i*(skip+1)+start], startBitSize,
	 * startPos, dest); startPos+=startBitSize; } }
	 **/
	 ...
	 *                   is interleaved with non-desirable data).
	 * @param countA     Number of total indices to store from.
	 * @param startPosIn First usable index in destination array(byte index =
	 *                   startPosIn/8, bit within that byte = startPosIn%8)
	 * @param dest       Destination array to store input values in. This array
	 *                   *must* be large enough to store all values or this method
	 *                   will fail in an undefined manner.
	 */
	private static void packInt(int[] inputArray, int bitSize, int startPosIn, int start, int skip, int countA,
			byte[] dest) {
		if (DEBUG_LEV > 30)
	...
	protected static int packIntByBitsToByteBoundary(int[] input, int[] inputBits, int inputIndex, int inputCount,
			int destPos, byte[] dest) {
	 ...
	 * @param destPos    First usable bit-level index in destination array(byte
	 *                   index = startPosIn/8, bit within that byte = destPos)
	 * @param dest       Destination array to store input values in. This array
	 *                   *must* be large enough to store all values or this method
	 *                   will fail in an undefined manner.
	 */
	protected static void packIntByBits(int[] inputA, int[] inputBits, int inputIndex, int inputCount, int destPos,
			byte[] dest) {
		int origInputIndex = inputIndex;
		inputIndex = packIntByBitsToByteBoundary(inputA, inputBits, inputIndex, inputCount, destPos, dest);
		if (destPos % 8 > 0)
			destPos = (destPos / 8 + 1) * 8;// put dest where we know it should be
	...
	protected static int compressIntArrayByBits(int[] input, int[] inputBits, int inCount, int inputIndex) {
		...
		for (int i = 0; i < inCount; i++) {
			...
			// if workingBits+bits > 32, shift down and OR into workingVal, add to output,
			// shift leftovers up and set workingVal
	...
	protected static void mergeFullOnByte(int[] input, int inCount, byte[] dest, int destIndex) {
		// System.err.println("mergeFullOnByte::begin inBitCount: "+inCount+" ::
		// destBitOffset: "+destIndex);
