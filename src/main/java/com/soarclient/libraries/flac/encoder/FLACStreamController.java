...
public class FLACStreamController {
	...
	/*
	 * position of header in output stream location(needed so we can update the
	 * header info(md5, minBlockSize, etc), once encoding is done
	 */
	...
	public void closeFLACStream(byte[] md5Hash, StreamConfiguration streamConfig) throws IOException {
		...
		try {
			...
			EncodedElement streamInfo = MetadataBlockStreamInfo.getStreamInfo(tempSC, minFrameSize, maxFrameSize,
					samplesInStream, md5Hash);
			if (out.canSeek()) {
	...
	/**
	 * Write the data stored in an EncodedElement to the output stream. All data
	 * will be written along byte boundaries, but the elements in the given list
	 * need not end on byte boundaries. If the data of an element does not end on a
	 * byte boundary, then the space remaining in that last byte will be used as an
	 * offset, and merged(using an "OR"), with the first byte of the following
	 * element.
	 *
	 * @param data
	 * @return
	...
	public void openFLACStream() throws IOException {
		...
		try {
			...
			EncodedElement streamInfo = MetadataBlockStreamInfo.getStreamInfo(streamConfig, minFrameSize, maxFrameSize,
					samplesInStream, md5Hash);
			...
			EncodedElement metadataBlockHeader = MetadataBlockHeader.getMetadataBlockHeader(false,
