...
public class EncodingConfiguration implements Cloneable {
	...
	/**
	 * Defines the options for channel configuration to use. LEFT and RIGHT channels
	 * refers to stereo audio as expected. INDEPENDENT channels refer to each
	 * channel encoded separately. SIDE channel is the difference of the LEFT and
	 * RIGHT channels(LEFT-RIGHT). MID channel is the integer average of LEFT and
	 * RIGHT channels( (LEFT+RIGHT)/2 ). Options using MID and/or SIDE channels may
	 * benefit encoding by taking advantage of similarities between the channels,
	 * and are available in FLAC for STEREO streams only. In general, ENCODER_CHOICE
	 * should be chosen.
	 */
	...
	/**
	 * Defines the various subframe types that may be used. If you don't know what
	 * these are, choose EXHAUSTIVE(for description of subframe types, see the flac
	 * format documentation at http://flac.sourceforge.net/format.html)
	 */
	public enum SubframeType {
		/**
		 * Constant subframe, do not choose unless you are sure you're encoding digital
		 * silence
		 */
		CONSTANT,
	...
	/**
	 * Constructor, uses defaults for all options. These defaults should be good for
	 * most purposes.
	 */
	...
	/**
	 * Set the channel configuration to use. Channel configuration refers to the way
	 * multiple channels are processed. See documentation for {@link ChannelConfig
	 ...
	 * {@link EncodingConfiguration#MIN_LPC_ORDER MIN_LPC_ORDER}), it will be set to
	 * the closest valid value instead.
	 * 
	...
	/**
	 * Set the maximum LPC order. If order given is out of the valid range (as
	 * defined by {@link EncodingConfiguration#MAX_LPC_ORDER MAX_LPC_ORDER} and
	 * {@link EncodingConfiguration#MIN_LPC_ORDER MIN_LPC_ORDER}), it will be set to
	 * the closest valid value instead.
	 * 
