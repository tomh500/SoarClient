...
public class MetadataBlockStreamInfo {
	...
	/*
	 * int minimumBlockSize = 4096;//32768;//16 bits used; 16 minimum valid int
	 * maximumBlockSize = 4096;//32768;//16 bits used; 65535 maximum valid int
	 * minimumFrameSize = 0;//24 bits used; zero implies unknown int
	 * maximumFrameSize = 0;//24 bits used; zero implies unknown int sampleRate =
	 * 8000;//4096;//20 bits used, but 655350Hz max. 0 is invalid. byte
	 * numberOfChannels = 1;//3 bits used byte bitsPerSample = 16;//5 bits used.
	 * values 4-32 valid long totalSamplesInStream = 0;//36 bits used. 0 implies
	 * unknown. byte[] md5Hash;
	 */
	/**
	...
	/**
	 * Create a FLAC StreamInfo metadata block with the given parameters. Because of
	 * the data stored in a StreamInfo block, this should generally be created only
	 ...
	 * @param samplesInStream Total number of inter-channel audio samples in FLAC
	 *                        stream.
	 * @param md5Hash         MD5 hash of the raw audio samples.
	 * @return EncodedElement containing created StreamInfo block.
	 */
	public static EncodedElement getStreamInfo(StreamConfiguration sc, int minFrameSize, int maxFrameSize,
			long samplesInStream, byte[] md5Hash) {
