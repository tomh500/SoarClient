...
public abstract class Subframe {
	...
	/**
	 * StreamConfiguration used for encoding. The same StreamConfiguration MUST be
	 * used throughout an entire FLAC stream
	 */
	protected StreamConfiguration sc;

	/**
	 * EncodingConfiguration used for encoding. This may be altered between frames,
	 * but must be the same for each subframe within that frame
	 */
	 ...
	 * Constructor. This configuration should create a decent encode, but may be
	 * altered if desired.
	 *
	 ...
	 * left and right samples of a stereo stream, while this SubFrame is only
	 * encoding the 'right' channel(channel 2). Therefore, “skip” would equal 2,
	 * resulting in the valid indices being only those where “index mod 2 equals 1”.
	 * 
	 ...
	 *                      differ from the StreamConfiguration's sample size,
	 *                      depending on the subframe used(i.e, the "side-channel"
	 *                      of a FLAC stream uses one extra bit compared to the
	 *                      input channels).
	 *
	 * @return number of encoded samples, or negative value indicating an error has
	 *         occurred.
	 *
	 */
	public abstract int encodeSamples(int[] samples, int count, int start, int skip, EncodedElement data, int offset,
			int bitsPerSample);
