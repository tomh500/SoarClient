...
public class StreamConfiguration implements Cloneable {
	...
	/**
	 * Maximum bits per sample allowed(FLAC spec allows 32, limited to 24 here due
	 * to limits in code)
	 */
	...
	/*
	 * is the currently set configuration valid? Encoders should not attempt to use
	 ...
	 * changed to match the audio characteristics(channel count, sample rate, sample
	 * size).
	 */
	...
	/**
	 * Constructor, allows setting of all options. In general, parameters given must
	 * match the input audio characteristics. minBlock and maxBlock may be set as
	 * desired, though minBlock is expected to be less than or equal to maxBlock. If
	 * minBlock or maxBlock is out of a valid range, it will be automatically
	 ...
	 * guessed and corrected, the value will be set to that given even if it is not
	 * valid.
	 * 
	...
	/**
	 * Set the bits per sample. Because this is not a value that may be guessed and
	 * corrected, the value will be set to that given even if it is not valid.
	 * 
	...
	/**
	 * Set the maximum block size to use. If this value is out of a valid range, it
	 * will be set to the closest valid value. User must ensure that this value is
	 * set above or equal to the minimum block size.
	 * 
	...
	/**
	 * Set the minimum block size to use. If this value is out of a valid range, it
	 * will be set to the closest valid value. User must ensure that this value is
	 * set below or equal to the maximum block size.
	 * 
	 ...
	 * ensure resulting FLAC streams are streamable. Not all options in that subset
	 * are defined by the StreamConfiguration class, however, but exist in the
	 * EncodingConfiguration class. Therefore, the alternative method
	 * {@link StreamConfiguration#isEncodingSubsetCompliant(EncodingConfiguration)
	 * isEncodingSubsetCompliant} should be checked as well to ensure the combined
	 * Stream/Encoding configurations are BOTH valid.
	 * 
	...
	/**
	 * Test if this StreamConfiguration and a paired EncodingConfiguration define a
	 * Subset compliant stream. FLAC defines a subset of options to ensure resulting
	 * FLAC streams are streamable.
	 * 
