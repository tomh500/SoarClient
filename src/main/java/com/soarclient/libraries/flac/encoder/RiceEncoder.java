...
public class RiceEncoder {
	...
	/**
	 * Constructor. A RiceEncoder object is used(as opposed to potentially faster
	 * static methods), for memory considerations; some temporary, dynamically
	 * created arrays are kept between calls to the encode methods to prevent
	 * frequently allocating and freeing similarly sized arrays.
	 */
	public RiceEncoder() {
	 ...
	 *
	 * @param useFiveBitParam Set TRUE if using a five-bit parameter size, FALSE for
	 *                        a four-bit parameter
	 * @param order           Specify order of partitions to be used(actual number
	 *                        of partitions will be 2^order.
	...
	public static int encodeRicePartitionEscaped(int[] values, int inputOffset, int inputStep, int inputCount,
			EncodedElement destEle, int bitParam, boolean fiveBitParam) {
		if (DEBUG_LEV > 0)
			System.err.println("RiceEncoder::encode : Begin");
		/*
		 * Currently, we're passing in an EncodedElement with a set byte[], and filling
		 * that array. We should therefore ensure that we're not writing too much to it.
		 * We *can* add another element to the given one if need.
		 */
	 ...
	 *                     the distribution of the input data(roughly speeking, each
	 *                     value will require at least bitParam+1 bits to save, so
	 *                     this value should reflect the average magnitude of input
	 *                     values.
	 ...
	 */
	public int encodeRicePartition(int[] values, int inputOffset, int inputStep, int inputCount, EncodedElement destEle,
	 ...
	 *                    interleaved data.
	 * @param inputCount  number of total values to encode
	 * @param bitParam    rice-parameter to use. This value should be based upon the
	 *                    distribution of the input data(roughly speeking, each
	 *                    value will require at least bitParam+1 bits to save, so
	 *                    this value should reflect the average magnitude of input
	 *                    values.
