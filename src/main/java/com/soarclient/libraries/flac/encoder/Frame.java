...
public class Frame {
	...
	/*
	 * Current EncodingConfiguration used. This must NOT be changed while a Frame is
	 * being encoded, but may be changed between frames
	 */
	...
	/**
	 * This method is used to set the encoding configuration. This configuration can
	 * be altered throughout the stream, but cannot be called while an encoding
	 * process is active.
	 *
	 * @param ec encoding configuration to use.
	 * @return <code>true</code> if configuration was changed. <code>false</code>
	 *         otherwise(i.e, and encoding process was active at the time of change)
	 */
	...
	/**
	 * Encodes samples into the appropriate compressed format, saving the result in
	 * the given “data” EncodedElement list. Encodes 'count' samples, from index
	 * 'start', to index 'start' times 'skip', where “skip” is the format that
	 * samples may be packed in an array. For example, 'samples' may include both
	 * left and right samples of a stereo stream. Therefore, “skip” would equal 2,
	 * resulting in the valid indices for the first channel being even, and second
	 * being odd.
	 * 
	 * @param samples the audio samples to encode. This array may contain samples
	 ...
	 * @param skip    the number of indices to skip between successive samples (for
	 *                use when channels are interleaved in the given array).
	 * @param result  the EncodedElement to attach encoded data to. Data in Encoded
	 ...
	 *         block-size is 4000, and it is stereo audio. There are 8000 samples in
	 *         this block, but the return value is “4000”. There is always an equal
	 *         number of samples encoded from each channel. This exists primarily to
	 *         support dynamic block sizes in the future; Pre-condition: none
	 *         Post-condition: Argument 'data' is the head of a list containing the
	 *         resulting, encoded data stream.
	 */
	public int encodeSamples_OLD(int[] samples, int count, int start, int skip, EncodedElement result,
			long frameNumber) {
		// System.err.println("FRAME::encodeSamples: frame#:"+frameNumber);
		if (DEBUG_LEV > 0) {
			System.err.println("FRAME::encodeSamples(...)");
			if (DEBUG_LEV > 10) {
				System.err.println("\tsamples.length:" + samples.length + ":count:" + count + ":start:" + start
						+ ":skip:" + skip + ":frameNumber:" + frameNumber);
			}
		}
		...
		EncodedElement header = frameHeader.createHeader(true, count, sc.getSampleRate(), chConf, sc.getBitsPerSample(),
	...
	public int encodeSamples(int[] samples, int count, int start, int skip, EncodedElement result, long frameNumber) {
		...
		testConstant = true;
		EncodedElement data = null;
		ChannelData[][] chanConfigData = this.getChannelsToEncode(samples, count, sc.getChannelCount(),
				sc.getBitsPerSample());
		...
		frameHeader.createHeader(true, count, sc.getSampleRate(), chConf, sc.getBitsPerSample(), frameNumber, channels,
				header);
		// result.setNext(header);
	...
	int encodeChannel(int[] samples, int count, int start, int skip, int offset, EncodedElement data,
			int channelBitsPerSample) {
		if (DEBUG_LEV > 0)
		...
		if (subframeType == EncodingConfiguration.SubframeType.VERBATIM) {
			...
			if (testConstant) {
				...
				conCount = constantSubframe.encodeSamples(samples, count, start, skip, constantEle, offset,
						channelBitsPerSample);
			}
	 ...
	 * number of compressed bits used). This is here for convenience, as the calling
	 * object may also loop through the resulting EncodingElement from the encoding
	 * process and sum the valid bits.
	 *
	 * @return an integer with value of the number of bits used in last encoding.
	 *         Pre-condition: none Post-condition: none
	 */
	...
	private EncodingConfiguration.ChannelConfig selectOptimalChannels(ChannelData[] independent, ChannelData[] midSide,
			int count) {
	...
	private static int encodeMidSide(int[] samples, int count, int start, int skip, EncodedElement data, int offset,
			Frame f) {
	...
	private static int allChannelDecorrelation(int[] samples, int count, int start, int skip, EncodedElement data,
			int offset, Frame f) {
		if (DEBUG_LEV > 0) {
