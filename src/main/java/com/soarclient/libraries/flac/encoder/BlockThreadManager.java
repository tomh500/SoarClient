 ...
 *
 *
 * BlockThreadManager accepts BlockEncodeRequest objects to encode. For each
 * Frame object given to encode with, a Thread is supplied. This thread will
 * take a BlockEncodeRequest, encode it, then give it back to the
 * BlockThreadManager The thread will then take another BlockencodeRequest, and
 * repeat. If no block encode requests are available, it will block till
 * available.
 *
 * 
 * The main thread of the BlockThreadManager will be waiting for additions to
 * the finished queue. If the item is not the next in line to be written to
 * file, it will be saved temporarily. If it is the next item to be written, it
 * will be given back to the FLACEncoder to be written to output, and any saved
 * objects will be searched to see if more can be written. The main thread will
...
public class BlockThreadManager implements Runnable {
	...
	/*
	 * requests that have just finished encoding, and are ready for writing. Since
	 * run() polls against this for newly finished requests, requests recieved out
	 * of order will be temporarily stored in finishedRequestStore
	 */
	LinkedBlockingQueue<BlockEncodeRequest> finishedEncodeRequests = null;

	/*
	 * pending requests in the order that we must pass them to the FLACEncoder. The
	 * top object is moved to nextTarget until it is found in finishedEncodeRequests
	 ...
	 * should remain valid and unchanged so long as blocks are encoding or queued.
	 * It may therefore be used to monitor/interrupt, an encode process.
	 */
	...
	/*
	 * Must be false if we've been explicitly told to stop. Adding new requests will
	 * reset this value to true
	 */
	...
	/**
	 * Get total number of BlockEncodeRequests added to this manager, but not yet
	 * passed back to the FLACEncoder object.
	 * 
	...
	/**
	 * This function is used to help control flow of BlockEncodeRequests into this
	 * manager. It will block so long as their is at least as many unprocessed
	 * blocks waiting to be encoded as the value given.
	 *
	...
	/**
	 * Add a Frame to this manager, which it will use to encode a block. Each Frame
	 * added allows one more thread to be used for encoding. At least one Frame must
	 * be added for this manager to encode.
	 * 
	...
	/**
	 * Notify this manager that a FrameThread has ended it's run() method, returning
	 * the FrameThread object to the manager for use in future Threads.
	 *
	...
	/**
	 * Get a BlockEncodeRequest object which is queued for encoding, pausing for up
	 * to 0.5 seconds till one is available. It is expected that this object will
	 * later(after encoding to a FLAC frame) be returned to this manager through the
	 * returnFinishedRequest method. Failure to return the finished object will
	 * cause the encoding process to hang.
	 *
	...
	/**
	 * Used to return a finished BlockEncodeRequest from a FrameThread. This must
	 * only be called with a finished request, which was originally added to this
	 * manager through the addRequest() method.
	 *
	 ...
	 * FLACEncoder for finalizing. If no request is finished, or currently assigned
	 * to an encoding thread, will timeout after 0.5 seconds and end.
	 */
	public void run() {
	 ...
	 * function that should *ever* call managerThread.start(). We should call this
	 * at any time a managerThread may be needed but not started. For example, after
	 * returning a BlockEncodeRequest from an encoding thread.
	 */
	...
	/**
	 * Add a BlockEncodeRequest to the manager. This will immediately attempt to
	 * assign a request to an encoding thread(which may not occur if no threads are
	 * currently available). Requests are passed back to the currently set
	 * FLACEncoder object when finished and ready to be written to output.
	 *
