...
public class FLAC_FileEncoder {
	...
	public enum Status {
		...
		/**
		 * internal error is something that went haywire that was discovered due to
		 * internal sanity checks. A problem in API.
		 */
		INTERNAL_ERROR,
	 ...
	 * StreamConfiguration configuration and default EncodingConfiguration. Thread
	 ...
	 * characteristics(number of channels, sample rate, and sample size), will be
	 * set to match the input file at encode time, so needn't be set in the given
