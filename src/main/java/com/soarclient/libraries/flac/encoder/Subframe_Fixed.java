...
public class Subframe_Fixed extends Subframe {
	/**
	 * For debugging: Higher values equals greater output, generally in increments
	 * of 10
	 */
	 ...
	 * later be changed, a new Subframe object must be created as well.
	 *
	...
	public int encodeSamples(int[] samples, int count, int start, int skip, int offset, int unencSampleSize) {
		...
		for (int i = 0; i < 5; i++) {
			...
			// lowOrderBits[i]++;//DOUBLE CHECK VALIDITY OF THIS. Decreases the bits needed,
			// but "shouldn't"
		...
		_errors = errors;
		_totalBits = unencSampleSize * order + 8
				+ RiceEncoder.calculateEncodeSize(errors, errorOffset, errorStep, errorCount, lowOrderBits[order]);
	 ...
	 * EncodedElement, properly packed at the bit-level to be added directly to a
	 * FLAC stream.
	 *
	...
	public EncodedElement getData(EncodedElement dataEle) {
		...
		/*
		 * for(int i = 0; i < errorCount; i++) { int error =
		 * errors[errorOffset+i*errorStep]; if(error >= 32767 || error <= -32767)
		 * System.err.println("Error Bound issue?: " + error); }
		 */
		rice.encodeRicePartition(_errors, _errorOffset, _errorStep, _errorCount, dataEle, lowOrderBits[_order],
				fiveBitParam);
	...
	public int encodeSamples(int[] samples, int count, int start, int skip, EncodedElement dataEle, int offset,
			int unencSampleSize) {
	...
	/*
	 * private int[] calculatePartitionsCount(int[] errors, int errorOffset, int
	 * errorStep, int errorCount) { int maxPartitions = 8; int[][] sums = new
	 * int[(int)Math.pow(2,maxPartitions)][]; for(int i = 0; i < maxPartitions; i++)
	 * { sums[i] = new int[(int)Math.pow(2, i)]; } int[] counts = new
	 * int[maxPartitions]; int[] usedPartitions = new int[maxPartitions]; int[]
	 * lastPartitionCount = new int[maxPartitions]; for(int i = 0; i <
	 * maxPartitions; i++) { counts[i] = errorCount/sums[i].length; if(errorCount %
	 * sums[i].length != 0) counts[i]++; usedPartitions[i] = errorCount/counts[i];
	 * if(errorCount %counts[i] != 0) usedPartitions[i]++; } for(int i = 0; i <
	 * maxPartitions; i++) { int temp = errorCount+errorOffset/ } int[] sizes = new
	 * int[maxPartitions];
	 * 
	 * int temp = 0; double log2 = Math.log(2); for(int i = 0; i < errorCount; i++)
	 * { temp = errors[errorOffset+i*errorStep]; if(temp < 0) temp = -temp; for(int
	 * x = 0; x < maxPartitions; x++) { float destDiv = i/counts[x]; int destIndex =
	 * (int)(i/destDiv); sums[x][destIndex] += temp; }
	 * 
	 * } //sum up all bit sizes per partition, choose best size. for(int i = 0; i <
	 * maxPartitions; i++) { int tempTotal = 0; sizes[i] = 0;
	 * 
	 * for(int x = 0; x < usedPartitions[i]; x++) { tempTotal =
	 * (int)(Math.log(sums[i][x])/Math.log(2)); float destDiv =
	 * (float)errorCount/sums[x].length; int destCount = errorCount/sums[x].length;
	 * int destIndex = (int)(i/destDiv); sizes[i] += tempTotal+4+; } } return
	 * results; }
	 */
