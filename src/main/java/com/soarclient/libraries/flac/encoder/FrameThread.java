...
public class FrameThread implements Runnable {
	 ...
	 * @param manager BlockThreadManager to use as the BlockEncodeRequest source and
	 *                destination.
	 */
	 ...
	 * BlockThreadManager, encode the block, return it to the manager, then repeat.
	 * If no BlockEncodeRequest is available, or if it recieves a request with the
	 * "frameNumber" field set to a negative value, it will break the loop and end,
	 * notifying the manager it has ended.
	 *
	 */
	public void run() {
		...
		synchronized (this) {
			...
			while (ber != null && process) {
				if (ber.frameNumber < 0) {
					...
					ber.encodedSamples = frame.encodeSamples(ber.samples, ber.count, ber.start, ber.skip, ber.result,
							ber.frameNumber);
					ber.valid = true;
