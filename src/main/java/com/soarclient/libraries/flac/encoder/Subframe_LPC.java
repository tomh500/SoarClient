...
public class Subframe_LPC extends Subframe {
	...
	public int encodeSamples(int[] samples, int count, int start, int skip, int offset, int unencSampleSize) {
		...
		for (int i = maxOrder; i >= minOrder; i--) {
			...
			int tempTotalBits = partialEncodeLPC(samples, count, start, increment, lpcs[i], this, frameSampleSize);
	...
	public EncodedElement getData() {
		...
		writeLPC(_samples, _lastCount, _start, _increment, result, _frameSampleSize, _lowOrderBits, _precision, _shift,
				_quantizedCoeffs, _errors, _lpcOrder, rice);
	...
	private static void writeHeadersAndData(EncodedElement dataEle, int order, int[] coeff, int precision, int shift,
			int[] samples, int sampleSize, int start, int skip) {
		// write headers
	...
	/**
	 * Quantize coefficients to integer values of the given precision, and calculate
	 * the shift needed.
	 * 
	 * @param coefficients values to quantize. These values will not be changed.
	 * @param dest         destination for quantized values.
	 * @param order        number of values to quantize. First value skipped,
	 *                     coefficients array must be at least order+1 in length.
	 * @param precision    number of signed bits to use for coefficients(must be in
	 *                     range 2-15, inclusive).
	 * @return
	 */
	...
	private static void writeLPC(int[] samples, int count, int start, int increment, EncodedElement ele,
			int frameSampleSize, int riceParam, int precision, int shift, int[] coeffs, int[] errors, int order,
			RiceEncoder rice) {
		writeHeadersAndData(ele, order, coeffs, precision, shift, samples, frameSampleSize, start, increment - 1);
	...
	private static int partialEncodeLPC(int[] samples, int count, int start, int increment, LPC lpc,
			Subframe_LPC lpcSubframe, int frameSampleSize) {
		 ...
		 * LOGE_2*deviation/SQRT_2; tempLowOrderBits =
		 * (Math.ceil(Math.log(tempBits)/LOGE_2));
		 ...
		 * System.err.println("Error: "+(int)error/count); System.exit(0); }
		 * if(tempLowOrderBits < 1) tempLowOrderBits = 1; else if (tempLowOrderBits >
		 * frameSampleSize) { tempLowOrderBits = frameSampleSize; }
		 */
		int precision = 15;
		...
		int riceSize = RiceEncoder.calculateEncodeSize(lpcSubframe._errors, order, 1, count - order,
				(int) tempLowOrderBits);
