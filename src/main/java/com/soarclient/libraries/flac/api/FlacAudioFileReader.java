...
public class FlacAudioFileReader extends AudioFileReader {
	 ...
	 *                                          valid audio file data recognized by
	 *                                          the system.
	 ...
	 * @return an AudioInputStream object based on the audio file data pointed to by
	 *         the URL.
	 * @exception UnsupportedAudioFileException if the File does not point to a
	 *                                          valid audio file data recognized by
	 *                                          the system.
	 ...
	 *               constructed.
	 * @return an AudioInputStream object based on the audio file data contained in
	 *         the input stream.
	 * @exception UnsupportedAudioFileException if the File does not point to a
	 *                                          valid audio file data recognized by
	 *                                          the system.
	 ...
	 * @exception IOException                   if an I/O exception occurs.
	 */
	protected AudioFileFormat getAudioFileFormat(InputStream bitStream, long mediaLength)
		 ...
		 * UnsupportedAudioFileException( "missing ogg id!"); } // how many segments are
		 * there? segments = header[SEGOFFSET] & 0xFF; if (segments > 1) { throw new
		 * UnsupportedAudioFileException( "Corrupt Flac Header: more than 1 segments");
		 * } dis.readFully(header, OGG_HEADERSIZE, segments); baos.write(header,
		 * OGG_HEADERSIZE, segments); chksum = OggCrc.checksum(chksum, header,
		 * OGG_HEADERSIZE, segments); // get the number of bytes in the segment
		 * bodybytes = header[OGG_HEADERSIZE] & 0xFF; if (bodybytes != FLAC_HEADERSIZE)
		 * { throw new UnsupportedAudioFileException( "Corrupt Flac Header: size=" +
		 * bodybytes); } // read the Flac header dis.readFully(header, OGG_HEADERSIZE +
		 * 1, bodybytes); baos.write(header, OGG_HEADERSIZE + 1, bodybytes); chksum =
		 * OggCrc.checksum(chksum, header, OGG_HEADERSIZE + 1, bodybytes); // make sure
		 * its a Flac header if (!FLACID.equals(new String(header, OGG_HEADERSIZE + 1,
		 * 8))) { throw new UnsupportedAudioFileException(
		 ...
		 * OGG_HEADERSIZE + 1 + 60) == 1; // Checksum if (chksum != origchksum) throw
		 * new IOException("Ogg CheckSums do not match"); // Calculate frameSize if
		 * (!vbr) { // Frames size is a constant so: // Read Comment Packet the Ogg
		 * Header of 1st data packet; // the array table_segment repeats the frame size
		 * over and over. } // Calculate frameRate if (mode >= 0 && mode <= 2 && nframes
		 * > 0) { frameRate = ((float) sampleRate) / ((mode == 0 ? 160f : (mode == 1 ?
		 * 320f : 640f)) * ((float) nframes)); }
		 */
		try {
	 ...
	 * @exception IOException                   if an I/O exception occurs.
	 */
	protected AudioInputStream getAudioInputStream(InputStream inputStream, long mediaLength)
			throws UnsupportedAudioFileException, IOException {
