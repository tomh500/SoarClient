package com.soarclient.libraries.flac.api;
...
import com.soarclient.libraries.flac.util.RingBuffer;
...
public abstract class RingedAudioInputStream extends AudioInputStream {
	 ...
	 * This value is always in the range 0 through <code>buf.length</code>; elements
	 * <code>buf[0]</code> through <code>buf[count-1]</code> contain buffered input
	 * data obtained and filtered from the underlying inputstream.
	 */
	// protected int count;
	 ...
	 * <p>
	 * This value is always in the range 0 through <code>count</code>. If it is less
	 * than <code>count</code>, then <code>buf[pos]</code> is the next byte to be
	 * supplied as input. If it is equal to <code>count</code>, then the next
	 * <code>read</code> or <code>skip</code> operation will require more bytes to
	 * be read and filtered from the underlying inputstream.
	 * 
	 * @see #buf
	 */
	// protected int pos;
	 ...
	 * <p>
	 * This value is always in the range -1 through <code>pos</code>. If there is no
	 * marked position in the inputstream, this field is -1. If there is a marked
	 * position in the inputstream, then <code>buf[markpos]</code> is the first byte
	 ...
	 * with suitable adjustments to the values of
	 * <code>count</code>,<code>pos</code>, and <code>markpos</code>); they may not
	 * be discarded unless and until the difference between <code>pos</code> and
	...
	/**
	 * The index one greater than the index of the last valid byte in the unfiltered
	 * data buffer. This value is always in the range 0 through
	...
	/**
	 * The current position in the unfiltered data buffer. This is the index of the
	 * next character to be read from the <code>prebuf</code> array.
	 * <p>
	...
	/**
	 * Creates a <code>RingedAudioInputStream</code> with the specified buffer size,
	 * and saves its argument, the inputstream <code>in</code> for later use. An
	...
	public RingedAudioInputStream(InputStream in, AudioFormat format, long length, int size, int presize) {
		...
		if (format.getFrameSize() > 0)
			buffer.resize(format.getFrameSize() * 2);
	}
	 ...
	 */
	/*
	 * protected void makeSpace() { if (markpos < 0) pos = 0; // no mark: throw away
	 * the buffer else if (pos >= buf.length) // no room left in buffer if (markpos
	 * > 0) { // can throw away early part of the buffer int sz = pos - markpos;
	 * System.arraycopy(buf, markpos, buf, 0, sz); pos = sz; markpos = 0; } else if
	 * (buf.length >= marklimit) { markpos = -1; // buffer got too big, invalidate
	 * mark pos = 0; // drop buffer contents } else { // grow buffer int nsz = pos *
	 * 2; if (nsz > marklimit) nsz = marklimit; byte[] nbuf = new byte[nsz];
	 * System.arraycopy(buf, 0, nbuf, 0, pos); buf = nbuf; } count = pos; }
	 */
	 ...
	 * returns the sum of the the number of bytes remaining to be read in the buffer
	 * (<code>count - pos</code>). The result of calling the <code>available</code>
	 * method of the underlying inputstream is not used, as this data will have to
	 * be filtered, and thus may not be the same size after processing (although
