...
import com.soarclient.libs.sodium.client.world.biome.BiomeCache;
import com.soarclient.libs.sodium.client.world.biome.BiomeColorCache;
import com.soarclient.libs.sodium.client.world.cloned.ChunkRenderContext;
import com.soarclient.libs.sodium.client.world.cloned.ClonedChunkSection;
import com.soarclient.libs.sodium.client.world.cloned.ClonedChunkSectionCache;
...
public class WorldSlice implements SodiumBlockAccess {
	...
	private static final int NEIGHBOR_CHUNK_RADIUS = MathHelper.roundUp(2, 16) >> 4;
		...
		for (int x = 0; x < SECTION_LENGTH; x++) {
			for (int y = 0; y < SECTION_LENGTH; y++) {
				for (int z = 0; z < SECTION_LENGTH; z++) {
					...
					this.unpackBlockData(this.blockStatesArrays[idx], this.sections[idx], context.volume());
	...
	public IBlockState getBlockState(int x, int y, int z) {
		if (!blockBoxContains(this.volume, x, y, z)) {
			...
			return this.getBlockStateRelative(relX, relY, relZ);
		}
	}
	...
	public IBlockState getBlockStateRelative(int x, int y, int z) {
		...
		int blockIdx = getLocalBlockIndex(x & 15, y & 15, z & 15);
		return this.blockStatesArrays[sectionIdx][blockIdx];
	}
	...
	public int getCombinedLight(BlockPos pos, int ambientLight) {
		...
		if (y >= 0 && y < 256 && x >= -30000000 && z >= -30000000 && x < 30000000 && z < 30000000) {
			int skyBrightness = this.getLightFromNeighborsFor(EnumSkyBlock.SKY, pos);
			int blockBrightness = this.getLightFromNeighborsFor(EnumSkyBlock.BLOCK, pos);
			...
			return this.defaultLightValues[0] << 20 | ambientLight << 4;
		}
	}
	...
	public BiomeGenBase getBiomeGenForCoords(BlockPos pos) {
		int x = pos.getX() - this.baseX >> 4;
		int z = pos.getZ() - this.baseZ >> 4;
		ClonedChunkSection section = this.sections[getLocalChunkIndex(x, z)];
		return section != null ? section.getBiomeForNoiseGen(pos.getX() & 15, pos.getZ() & 15) : BiomeGenBase.plains;
	}
