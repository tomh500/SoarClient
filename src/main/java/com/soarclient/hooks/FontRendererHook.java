...
import com.soarclient.libraries.patcher.font.hash.impl.StringHash;
import com.soarclient.libraries.patcher.font.text.CachedString;
...
public final class FontRendererHook {
	...
	private void establishSize() {
		...
		try (final InputStream stream = mc.getResourceManager().getResource(fontRenderer.getLocationFontTexture())
				.getInputStream()) {
			...
			try (final InputStream stream = mc.getResourceManager()
				...
				if (i == 0x0d) {
					try (final InputStream sustream = FontRendererHook.class
						...
						fontRenderer.getGlyphWidth()[0xD9E] = 14;
					}
				}
			} catch (Exception ignored) {
			}
	...
	public boolean renderStringAtPos(String text, boolean shadow) {
		...
		if (this.fontRenderer.getRenderEngine() == null) {
		...
		float posX = this.fontRenderer.getPosX();
		float posY = this.fontRenderer.getPosY();
		this.fontRenderer.setPosY(0.0f);
		this.fontRenderer.setPosX(0.0f);

		float red = this.fontRenderer.getRed();
		float green = this.fontRenderer.getGreen();
		float blue = this.fontRenderer.getBlue();
		float alpha = this.fontRenderer.getAlpha();
		...
		if (cachedString != null) {
			...
			this.fontRenderer.setPosX(posX + cachedString.getWidth());
			this.fontRenderer.setPosY(posY + cachedString.getHeight());
			return true;
		}
		...
		final CachedString value = new CachedString(text, list, this.fontRenderer.getPosX() - posX,
				this.fontRenderer.getPosY() - posY);
		...
		for (int messageChar = 0; messageChar < text.length(); ++messageChar) {
			...
			if (letter == 167 && messageChar + 1 < text.length()) {
				...
				if (styleIndex < 16) {
					this.fontRenderer.setStrikethroughStyle(false);
					this.fontRenderer.setUnderlineStyle(false);
					this.fontRenderer.setItalicStyle(false);
					this.fontRenderer.setRandomStyle(false);
					this.fontRenderer.setBoldStyle(false);
					...
					int currentColor = fontRenderer.getColorCode()[styleIndex];

					this.fontRenderer.setTextColor(currentColor);
					...
					this.fontRenderer.setRandomStyle(true);
					obfuscated = true;
				} else if (styleIndex == 17) {
					this.fontRenderer.setBoldStyle(true);
				} else if (styleIndex == 18) {
					this.fontRenderer.setStrikethroughStyle(true);
				} else if (styleIndex == 19) {
					this.fontRenderer.setUnderlineStyle(true);
				} else if (styleIndex == 20) {
					this.fontRenderer.setItalicStyle(true);
				} else {
					this.fontRenderer.setRandomStyle(false);
					this.fontRenderer.setBoldStyle(false);
					this.fontRenderer.setStrikethroughStyle(false);
				...
				if (this.fontRenderer.isRandomStyle() && index != -1) {
				...
				if (small) {
					this.fontRenderer.setPosX(fontRenderer.getPosX() - boldWidth);
					this.fontRenderer.setPosY(fontRenderer.getPosY() - boldWidth);
				}

				float effectiveWidth = this.renderChar(letter, this.fontRenderer.isItalicStyle());
				if (small) {
					this.fontRenderer.setPosX(fontRenderer.getPosX() + boldWidth);
					this.fontRenderer.setPosY(fontRenderer.getPosY() + boldWidth);
				}

				if (this.fontRenderer.isBoldStyle()) {
					this.fontRenderer.setPosX(fontRenderer.getPosX() + boldWidth);
					...
					this.renderChar(letter, this.fontRenderer.isItalicStyle());
					this.fontRenderer.setPosX(fontRenderer.getPosX() - boldWidth);
				...
				if (this.fontRenderer.isStrikethroughStyle()) {
					adjustOrAppend(strikethrough, this.fontRenderer.getPosX(), effectiveWidth, value.getLastRed(),
							value.getLastGreen(), value.getLastBlue(), value.getLastAlpha());
				}

				if (this.fontRenderer.isUnderlineStyle()) {
					adjustOrAppend(underline, this.fontRenderer.getPosX(), effectiveWidth, value.getLastRed(),
							value.getLastGreen(), value.getLastBlue(), value.getLastAlpha());
				}

				this.fontRenderer.setPosX(fontRenderer.getPosX() + (int) effectiveWidth);
			}
		}
		...
		if (hasStyle) {
			...
			for (final RenderPair renderPair : strikethrough) {
				...
				GL11.glVertex2f(renderPair.posX, this.fontRenderer.getPosY() + 4.0f);
				GL11.glVertex2f(renderPair.posX + renderPair.width, this.fontRenderer.getPosY() + 4.0f);
				GL11.glVertex2f(renderPair.posX + renderPair.width, this.fontRenderer.getPosY() + 3.0f);
				GL11.glVertex2f(renderPair.posX, this.fontRenderer.getPosY() + 3.0f);
			}

			for (final RenderPair renderPair : underline) {
				GlStateManager.color(renderPair.red, renderPair.green, renderPair.blue, renderPair.alpha);
				GL11.glVertex2f(renderPair.posX - 1.0f, this.fontRenderer.getPosY() + 9);
				GL11.glVertex2f(renderPair.posX + renderPair.width, this.fontRenderer.getPosY() + 9);
				GL11.glVertex2f(renderPair.posX + renderPair.width, this.fontRenderer.getPosY() + 9 - 1.0F);
				GL11.glVertex2f(renderPair.posX - 1.0f, this.fontRenderer.getPosY() + 9 - 1.0F);
			}
		...
		value.setWidth(this.fontRenderer.getPosX());

		this.fontRenderer.setPosY(posY + value.getHeight());
		this.fontRenderer.setPosX(posX + value.getWidth());
	...
	public float renderChar(char ch, boolean italic) {
		if (ch == 32 || ch == 160) {
			...
			return charIndex != -1 && !this.fontRenderer.getUnicodeFlag()
					? this.renderDefaultChar(charIndex, italic, ch)
	...
	private float renderDefaultChar(int characterIndex, boolean italic, char ch) {
		...
		GL11.glVertex2f(this.fontRenderer.getPosX() + (float) italicStyle, this.fontRenderer.getPosY());

		GL11.glTexCoord2f(characterX / fontTexWidth, (characterY + uvHeight) / fontTexHeight);
		GL11.glVertex2f(this.fontRenderer.getPosX() - (float) italicStyle, this.fontRenderer.getPosY() + 7.99F);
		...
		GL11.glVertex2f(this.fontRenderer.getPosX() + smallCharWidth - 1.0F - (float) italicStyle,
				this.fontRenderer.getPosY() + 7.99F);

		GL11.glTexCoord2f((characterX + uvWidth - offset) / fontTexWidth, characterY / fontTexHeight);
		GL11.glVertex2f(this.fontRenderer.getPosX() + smallCharWidth - 1.0F + (float) italicStyle,
				this.fontRenderer.getPosY());
	...
	private Pair<Float, Float> getUV(char characterIndex) {
		...
		final int glyphWidth = this.fontRenderer.getGlyphWidth()[characterIndex] >>> 4;
	...
	private float renderUnicodeChar(char ch, boolean italic) {
		if (this.fontRenderer.getGlyphWidth()[ch] == 0) {
			return 0.0F;
		} else {
			final Pair<Float, Float> uv = getUV(ch);
			final int glyphX = this.fontRenderer.getGlyphWidth()[ch] >>> 4;
			final int glyphY = this.fontRenderer.getGlyphWidth()[ch] & 15;
			...
			GL11.glVertex2f(this.fontRenderer.getPosX() + italicStyle, this.fontRenderer.getPosY());

			GL11.glTexCoord2f(uv.getFirst(), uv.getSecond() + v / fontTexHeight);
			GL11.glVertex2f(this.fontRenderer.getPosX() - italicStyle, this.fontRenderer.getPosY() + 7.99F);

			final float texAdj = combinedGlyphSize + .5f;
			GL11.glTexCoord2f(uv.getFirst() + texAdj / fontTexHeight, uv.getSecond() + v / fontTexHeight);
			GL11.glVertex2f(this.fontRenderer.getPosX() + combinedGlyphSize / 2.0F - italicStyle,
					this.fontRenderer.getPosY() + 7.99F);
