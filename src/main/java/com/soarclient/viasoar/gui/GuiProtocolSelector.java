...
public class GuiProtocolSelector extends GuiScreen {
	...
	public GuiProtocolSelector(final GuiScreen parent) {
		this(parent, false, (version, unused) -> {
			ViaSoarCommon.getManager().setTargetVersion(version);
		});
	}
	...
	public void initGui() {
		...
		list = new SlotList(mc, width, height, 3 + 3 + (fontRendererObj.FONT_HEIGHT + 2) * 3, height - 30,
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(fontRendererObj, "https://github.com/MolokyMC/ViaForgeMCP", width / 2,
				(fontRendererObj.FONT_HEIGHT + 2) * 2 + 3, -1);
	...
	class SlotList extends GuiSlot {
		...
		protected void drawSlot(int index, int x, int y, int slotHeight, int mouseX, int mouseY) {
			final ProtocolVersion targetVersion = ViaSoarCommon.getManager().getTargetVersion();
			final ProtocolVersion version = ProtocolVersionList.getProtocolsNewToOld().get(index);

			String color;
			if (targetVersion == version) {
				color = GuiProtocolSelector.this.simple ? ChatFormatting.GOLD.toString()
						: ChatFormatting.GREEN.toString();
			} else {
				color = GuiProtocolSelector.this.simple ? ChatFormatting.WHITE.toString()
